...
def painted_coat(ctx, groups, materials, settings):
    ...
    body_roles = ('body', 'hands', 'feet', 'tail') if 'bodyFur' in ctx.images else ()
    for ob in ctx.scene.objects:
        ...
        for vertex, item in zip(ob.data.vertices, flow.data):
            ...
            if mantle:
                # Tangential locks flow from the shoulders toward both wrists.
                x, y = abs(p.x), p.y + .10
    ...
    for role, material in materials.items():
        ...
        head_fur = 'headFur' in ctx.images and (role in ('head', 'headTufts', 'dark') or role.startswith('earFur'))
        ...
        for node in nodes:
            if node.type == 'TEX_IMAGE':
                ...
                elif head_fur: node.image = ctx.images['headFur']
                ...
                if source.type == 'RGBTOBW':
                    ...
                    if head_fur:
                        node.inputs['From Min'].default_value = .07
                        node.inputs['From Max'].default_value = .45
                        node.inputs['To Min'].default_value = .48
                        node.inputs['To Max'].default_value = 1.10
                    elif role in body_roles:
                        node.inputs['From Min'].default_value = .07
                        node.inputs['From Max'].default_value = .45
                        node.inputs['To Min'].default_value = .70
                        node.inputs['To Max'].default_value = 1.15
        ...
        if role == 'head' or role.startswith('earFur'):
            ...
            if head_fur: dark_fur.inputs[1].default_value = (.032, .033, .034, 1)
            ...
            if head_fur:
                band_values.inputs['From Min'].default_value = .07
                band_values.inputs['From Max'].default_value = .45
                band_values.inputs['To Min'].default_value = .45
                band_values.inputs['To Max'].default_value = 1.55
        ...
        if role == 'body' and body_roles:
            ...
            # the authored charcoal palette and broad normal-based form shading.
            ...
            lock_planes.inputs['From Min'].default_value = .07
            lock_planes.inputs['From Max'].default_value = .45
            lock_planes.inputs['To Min'].default_value = .95
            lock_planes.inputs['To Max'].default_value = 1.05
            ...
            groups.inputs['From Min'].default_value = .07
            groups.inputs['From Max'].default_value = .45
            groups.inputs['To Min'].default_value = .84
            groups.inputs['To Max'].default_value = 1.16
            links.new(texture_grey, groups.inputs['Value'])
            shade = nodes.new('ShaderNodeMixRGB')
            shade.blend_type = 'MULTIPLY'; shade.inputs[0].default_value = .55
            links.new(original, shade.inputs[1]); links.new(groups.outputs[0], shade.inputs[2])
            original = shade.outputs[0]
        if role in ('head', 'tail', 'feet', 'dark', 'hands') or role.startswith('earFur') or (role == 'body' and body_roles):
        ...
        ao.samples = 32
        ao.only_local = role.startswith('earFur')
