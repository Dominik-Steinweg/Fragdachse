/**
 * Exportiert die Waldboden-HUD-Rahmen in einen kleinen Laufzeit-Atlas.
 *
 ...
 * Spalte). Ecken bleiben horizontal unverändert; nur ein ruhiges Stück Schiene wird gestreckt.
 * Die Quellhöhe wird auf das bestehende HUD-Raster normalisiert. Ein senkrecht gedrehtes Stück
 * der neutralen Holzschiene dient als dezenter Trenner für die Statusleiste.
 */
...
const SOURCE = 'waldboden-farbsaum-sheet.png';
...
const ROW_BANDS = [[36, 142], [144, 248], [252, 355], [368, 474], [478, 587], [592, 705], [710, 820], [828, 942]];
const CARD_COLUMN = [1160, 1512];
const STRIP_COLUMN = [24, 711];
/** Breite der unveränderten Eckstücke inklusive Efeu und Wurzelornament. */
const CAP = 64;
...
async function extract(left, top, width) {
  return sharp(original).extract({ left, top, width, height: SOURCE_H })
    .resize(width, FRAME_H, { fit: 'fill' }).toBuffer();
}
...
// Die neue Leiste hat eine durchgehend offene Innenfläche.
...
// Ein kurzes Stück der oberen Holzschiene, ohne zusätzliche Endornamente.
const DIVIDER_W = 10;
const divider = await sharp(original)
  .extract({ left: 300, top: stripBox.top + PAD_TOP, width: 48, height: DIVIDER_W })
  .rotate(90).extend({ top: 25, bottom: 25, left: 0, right: 0, background: '#00000000' })
  .png().toBuffer();
...
await writeFile(new URL('runtime/REPORT.md', assets), '# HUD frame export\n\n'
  + `Generated by \`node scripts/export-hud-frames.mjs\` from the untouched \`${SOURCE}\` (set-09-waldboden-farbsaum.png).\n\n`
