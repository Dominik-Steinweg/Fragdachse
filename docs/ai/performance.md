...
Fels-Decals und eine grossraeumige Farbvariation liegen gemeinsam in einem gebackenen `rockDecalLayer` knapp ueber den live-zerstoerbaren 47-Blob-Felsen. `ArenaBuilder.rebuildRockDecals()` filtert beide ueber die aktiven Fels-IDs; die Farbkopien verwenden ein deterministisches Feld aus breiten elliptischen Waeschen mit weichen Kanten, statt jedes Tile unabhaengig zu faerben. Ein ueber eine Zellgrenze laufendes Decal verschwindet deshalb konservativ vollstaendig, sobald einer seiner Traeger zerstoert wurde. Kleine Decals bleiben bei 26 px, groessere halbtransparente Moos-/Flechtenflaechen werden bei 48 px mit ihrer tatsaechlichen rotierten Bounds-Menge verknuepft. `RockVisualHelper.markObstaclesDirty()` sammelt dieselbe Hindernisaenderung wie Schatten und Light-Occluder und backt das Layer hoechstens einmal am Frame-Ende neu. So entsteht im Frame nur ein gebackenes Layer-Objekt, ohne per-Fels-Farbkopien im Renderwalk.
