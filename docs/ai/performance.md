...
Fels-Decals und die nicht-periodische Materialstoerung liegen in getrennten gebackenen Layern knapp ueber den 47-Blob-Felsen: `ArenaBuilder.rebuildRockOverlays()` filtert die Decals ueber aktive Fels-IDs und baut den Mottle-Layer bei Hindernisaenderungen neu. Der Mottle-Pfad verwendet deterministische Zellkoordinaten, wird mit `bakeRockSurfaceMottle()` sowohl fuer die Arena als auch fuer die Lobby-Vorschau erzeugt und per inverser Stanzmaske exakt auf die echte Blob-Silhouette beschnitten. Ein ueber eine Zellgrenze laufendes Decal verschwindet deshalb konservativ vollstaendig, sobald einer seiner Traeger zerstoert wurde. Kleine Decals bleiben bei 26 px, groessere halbtransparente Moos-/Flechtenflaechen werden bei 48 px mit ihrer tatsaechlichen rotierten Bounds-Menge verknuepft. `RockVisualHelper.markObstaclesDirty()` sammelt dieselbe Hindernisaenderung wie Schatten und Light-Occluder und backt die Layer hoechstens einmal am Frame-Ende neu. So entstehen im Frame nur gebackene Layer-Objekte, ohne per-Fels-Farbkopien im Renderwalk.
