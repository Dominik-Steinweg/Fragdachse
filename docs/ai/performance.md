...
Fels-Decals liegen in einem getrennten gebackenen `rockDecalLayer` knapp ueber den live-zerstoerbaren 47-Blob-Felsen. `ArenaBuilder.rebuildRockDecals()` filtert sie ueber ihre getragene Fels-ID-Menge; ein ueber eine Zellgrenze laufendes Decal verschwindet deshalb konservativ vollstaendig, sobald einer seiner Traeger zerstoert wurde. Kleine Decals bleiben bei 26 px, groessere halbtransparente Moos-/Flechtenflaechen werden bei 48 px mit ihrer tatsaechlichen rotierten Bounds-Menge verknuepft. `RockVisualHelper.markObstaclesDirty()` sammelt dieselbe Hindernisaenderung wie Schatten und Light-Occluder und backt das Layer hoechstens einmal am Frame-Ende neu. So entstehen keine per-Fels-Decal-Images im Renderwalk und keine schwebenden Riss-/Moosreste nach Retiling oder Zerstoerung.
