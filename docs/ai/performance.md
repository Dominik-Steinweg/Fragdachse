...
Der Dirt-Boden ist rein statisch und macht den Großteil der Display-Liste aus (mehrere tausend Kacheln). `ArenaBuilder.bakeDirt()` erzeugt getinte 47-Blob-Kacheln, backt Basis und `BlobSurfaceMottle` einmalig in **eine** arenagroße RenderTexture (interne Kamera um den Arena-Offset gescrollt) und verwirft danach alle temporären Bilder, Mottle-Layer und Silhouette-Cutouts. So läuft pro Frame ein Objekt statt tausender durch Renderwalk und Transform-Schritt; die Kosten sind fix und qualitätsunabhängig. Zerstörbare Felsen bleiben bewusst außen vor und dynamisch.
...
Die zehn graubraunen Dirt-Unterlagen für Felszellen werden auf derselben Dirt-Zelle wie der Fels gebacken, aber im Dirt-Decal-Band unter `DEPTH.ROCKS` gehalten. Sie brauchen keinen dynamischen Renderer: Nach der Zerstörung des Felsobjekts werden sie automatisch sichtbar.
...
Fels-Decals und die nicht-periodische Materialstoerung liegen in getrennten gebackenen Layern knapp ueber den 47-Blob-Felsen: `ArenaBuilder.rebuildRockOverlays()` filtert die Decals über aktive Fels-IDs und baut den profilbasierten `BlobSurfaceMottle`-Layer bei Hindernisänderungen neu. Derselbe generische Pfad wird für Arena und Lobby-Vorschau verwendet und per inverser Stanzmaske exakt auf die echte Blob-Silhouette beschnitten; er ist ebenso für spätere dynamische Base-Profile verwendbar. `RockVisualHelper.markObstaclesDirty()` sammelt dieselbe Hindernisänderung wie Schatten und Light-Occluder und backt die Layer höchstens einmal am Frame-Ende neu. Sehr große 64px-Decals dürfen nur auf `core`-Zellen liegen und verwenden einen maximalen Offset von 2px; große 48px-Decals bleiben bei 4px, damit gedrehte Bounds innerhalb des 3×3-Kernverbunds bleiben.
