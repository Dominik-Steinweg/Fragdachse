...
- Die Kampagnenregistry enthält Sandbox 00 sowie genau die Kampagnenmaps 01 bis 17; Map 1 ist die geführte Vorstoß-Tutorialroute, Map 9 basisloses Survival, Map 14 Survival mit Stellung, Map 16 repel-assault und Map 17 destroy-hostile-bases mit optionalem Carry. Die technischen Persistent-Base-Foundation-Maps 18/19 sind entfernt.
...
Maps mit `persistentBase.baseId` verwenden die vorhandene authored Friendly-Main-Base als stabilen Anker; eine zweite persistente Basis wird nicht erzeugt. Persönliche Beiträge liegen im V4-Progress-Dokument als ownergebundene, map-relative Blueprints; der Host bildet daraus deterministisch den Composite-State aus Authored-Basis, base-owned Rewards, Host-Beitrag und stabil sortierten Gastbeiträgen. Der Host lädt, validiert, plant und materialisiert diesen Composite zu Rundenbeginn und commitet Änderungen erst nach einem Sieg.

Die aktive Zone wird aus dem gespeicherten Radius und dem gemeinsamen Anker-Grid berechnet. Der Generator reserviert unabhängig vom aktuellen Radius immer den maximalen Radius plus Clearance für Felsen, Bäume, Tracks, Hazards, Spawns und andere authored/prozedurale Hindernisse. Map 11 verwendet den Coop-Base-Middle-Anker, Maps 12–14 den Coop-Base-Rear-Anker; Maps 15–17 haben standardmäßig keine aktive Persistent Base.
