...
- **Auch ein bildschirmfestes Vollbild-Composite folgt der Kameramatrix.** Phasers Kamerawackeln verschiebt Objekte mit `scrollFactor(0)` ebenfalls; eine exakt viewportgroße Lightmap legt dadurch unbeleuchtete Randstreifen frei. `LightingSystem` rendert deshalb eine ausgerichtete Randreserve für den stärksten unterstützten Shake und den maximalen Render-Scale des Qualitätsprofils. Lichtpositionen und Culling verwenden denselben Versatz.
...
- **Die Uhrzeit steht pro Map fest und wechselt nicht während der Runde.** `coopDefenseMaps.json` trägt sie als `"HH:MM"`; alle übrigen Modi bleiben beim Mittag. Host und Client leiten sie lokal aus der replizierten Map-ID ab – kein eigener Netzwerkpfad. Der Debug-Regler auf `M` wirkt bewusst nur lokal.
...
- **Ein Hitscan-/Strahl-Effekt leuchtet dort, wo er tatsächlich gerendert wird.** Das ASMD-Primärfeuer und die Kugelgewitter-Kettenblitze zeichnen beide über `AsmdPrimaryRenderer.playTracer()`; deshalb sitzt ihr `electricArc`-Licht dort und nicht in `EffectSystem.playHitscanTracer()`, das den ASMD-Zweig bewusst überspringt, um kein Doppellicht zu erzeugen. Wer einen Strahl-Effekt beleuchtet, sucht erst die eine Stelle, die beide Aufrufer speist.
