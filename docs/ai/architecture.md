...
- Ihre `actionPolicy.playerRelationships` ist `free-for-all`: Verschiedene interaktive Spieler sind Gegner, unabhängig von der in der Lobby ausgewählten späteren Match-Art. Dafür wird weder eine Activity noch ein Lobby-Team erzeugt.
...
Sie beantwortet die teilnahmebezogenen Fragen über kleine Prädikate: `hasWorldRuntimeEntry()`, `maySendWorldInput()` (nur `interactive`) und `requiresLocalWorldPresentation()` (alles außer `none`). `WorldParticipationInput` kennt bewusst keinen Rundenbegriff. Wer nicht handeln darf, ist `observer` — auch ohne eigenen Runtime-Eintrag, denn ein Zuschauer wartet auf keine Figur.

World-Replikation ist davon getrennt. `consumesWorldReplication()` (src/world/WorldReplication.ts) kombiniert die lokale `WorldParticipation` mit der auf diesem Peer erforderlichen `WorldPresentation`: Ein Teilnehmer konsumiert den Zustand für seine Runtime, eine `preview` trotz `Participation: none` für die passive Darstellung derselben laufenden World. Nur Replikation zu konsumieren erzeugt weder eine eigene Player-Runtime noch Input; Player-Entities werden ausschließlich für autoritativ `interactive` teilnehmende IDs erzeugt, die zugleich im Player-Snapshot stehen.
...
Der Attach ist atomar: lehnt ein Modul ab (`run` liefert `false`) oder wirft es, werden die bereits angehängten Module in umgekehrter Reihenfolge zurückgenommen — ein Spieler bleibt nie halb initialisiert. Der Detach läuft über dieselbe Feature-Entscheidung, aber nur für tatsächlich angehängte Spieler; wiederholtes Leave ist dadurch ein idempotenter No-op, und ein späteres Rejoin beginnt mit einem neuen Attach.
