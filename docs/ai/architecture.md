...
Die Übergangs- und Race-Verträge stehen in `tests/TransitionRaceCases.test.ts`: gleichzeitige Erstanforderungen werden auf eine World-Instanz zusammengeführt, Join-/Leave- und Activity-Ende bleiben ohne verwaiste Runtime, und ein verspätetes Attach einer bereits zerstörten Revision wird verworfen. World-Baselines und Loadout-/Placement-RPCs tragen bzw. prüfen ihre `worldRevision`; Nachrichten der alten Instanz erreichen die neue Runtime nicht.
