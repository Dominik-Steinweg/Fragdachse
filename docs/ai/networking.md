...
Zur Instanzidentität gehören auch die `parameters`: `isSameWorldInstance()` vergleicht sie feldweise über `WORLD_PARAMETER_FIELDS`, damit ein neu ergänzter World-Parameter nicht still aus der Identität fällt. Zwei Peers mit unterschiedlichen Parametern haben nicht dieselbe World gebaut. `definitionId` löst im Authoring-Registry auf, sofern sie nicht `isProceduralWorldDefinitionId()` erfüllt; PvP-Activities tragen bewusst die definitionslose Form `activity:<kind>`.
