...
Die Acquisition eines neuen Render-Chunks führt keinen synchronen Vollbake mehr aus. Sie reserviert ein unsichtbares Ziel und reiht dessen 128-px-Regionen in den gemeinsamen `ChunkBakeScheduler` der Scene ein; Ground, RockOverlay und statische Schatten teilen sich pro Frame ein kleines Zeitbudget. Sichtbare Chunks werden vor reinem Prefetch priorisiert, innerhalb der Bänder zählen Kameranähe und Bewegungsrichtung. Der Render-Prefetch liegt bewusst über dem alten 128-px-Erwerbsrand. Ein Chunk wird erst nach Abschluss aller Regionen sichtbar. Wird eine Region während eines ausstehenden Aufbaus dirty, wird genau diese Arbeitseinheit erneut eingereiht; verlässt der Chunk den Freigaberand, werden seine Jobs abgebrochen und die Ziele wie bisher über den Pool recycelt.
