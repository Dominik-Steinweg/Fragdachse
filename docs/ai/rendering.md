...
Die Acquisition eines neuen Render-Chunks führt keinen synchronen Vollbake mehr aus. Gepoolte Renderziele werden direkt übernommen; tatsächlich neue RenderTextures und Layer werden selbst als Jobs in den gemeinsamen `ChunkBakeScheduler` eingereiht und erst danach mit ihren 128-px-Regionen gebacken. Ground, RockOverlay und statische Schatten teilen sich pro Frame ein kleines Zeitbudget: reiner Prefetch liegt normalerweise bei etwa 1,5 ms, sichtbare unfertige Chunks oder dringende sichtbare Dirty-Arbeit dürfen bis etwa 4 ms nutzen. Der Scheduler hält zusammengehörige Acquisition-/Bake-Jobs möglichst bis zur Chunk-Fertigstellung zusammen, priorisiert Dirty-Rebuilds vor reinem Prefetch und berücksichtigt weiterhin Sichtbarkeit, Kameranähe und Bewegungsrichtung. Der Render-Prefetch liegt bewusst über dem alten 128-px-Erwerbsrand. Ein Chunk wird erst nach Abschluss aller Regionen sichtbar; beim normalen Full-Refresh bleibt er bis zum vollständigen Neuaufbau verborgen, damit kein Patchwork aus alten und neuen Regionen erscheint. Der statische Shadow-Profilwechsel durch `ShadowSystem.syncStaticProfile()` ist die bewusste Ausnahme: Mit `refreshAll({ preserveVisible: true })` bleibt ein bereits fertiger Chunk sichtbar, während seine Regionen schrittweise überschrieben werden; alte und neue Regionen können dabei kurz koexistieren. `isReadyForView()` meldet den Chunk wegen ausstehender Regionen trotzdem als nicht vollständig bereit. Wird eine Region während eines ausstehenden Aufbaus dirty, wird genau diese Arbeitseinheit erneut eingereiht; verlässt der Chunk den Freigaberand, werden seine Jobs abgebrochen und die Ziele wie bisher über den Pool recycelt.
...
Der Boden ist eine feste Reihenfolge gebackener Bänder: Gras (`DEPTH.GRASS`) < Dirt samt eingebackenem `BlobSurfaceMottle` (`DEPTH.DIRT`) < der aktuelle Persistent-Base-Kies-Blob (`DEPTH.PERSISTENT_BASE_GRAVEL`, nur auf entsprechend konfigurierten Karten) < Ground Cover (`DEPTH.GROUND_COVER`) < große Kies-Dekorstempel (`DEPTH.PERSISTENT_BASE_GRAVEL_DECORATION`) < Gleise (`DEPTH.TRACKS`) < Basiszonen (`DEPTH.BASES`) < Decals (`DEPTH.DECALS`). Der Kieszustand wird ausschließlich aus Seed, Persistent-Base-Anker und dem aktuell replizierten Radius abgeleitet; `PersistentBaseVisuals` besitzt dafür nur das optionale Platzierungs-Overlay, keine Terrain-Images.

Ein Render-Chunk sammelt seine Quellzellen mit Rand ein, nicht nur die eigenen: Dirt- und Persistent-Base-Kies-Autotiling sowie Ecktints müssen den **gesamten** Bestand der jeweiligen Schicht sehen, sonst liest jede Chunkgrenze wie eine Außenkante des Bodens; die Randfahne reicht um ihren Überhang über die eigene Zelle hinaus, die Materialstörung um ein Vielfaches davon. Kies-Dekore werden mit absoluten Zellkoordinaten und Seed deterministisch platziert, damit Radiusänderungen und Chunk-Recycling die inneren Pixel nicht neu würfeln.

Der LeafBlower erhält nach dem synchronen Arena-Aufbau genau einen opaken `TerrainColorSnapshot`. Er wird mit festem `scale = 4` aus mehreren 512×512-Scratch-Regionen aufgebaut; jede Region wird einmal gelesen und in das zusammenhängende RGB-Array kopiert, danach wird dasselbe Scratch-Ziel wiederverwendet. Die Darstellung folgt Grass, Background-Detail/MULTIPLY, Dirt samt Fringe/Corner-Tints/Mottle, Persistent-Base-Kies samt authored Dekoren, Ground Cover, Tracks, statischen Base-Zonen und Ground-Decals. Die Tile- und Sampling-Phase verwendet den globalen Welt-/Tileoffset, damit Regionsgrenzen nicht bei Texture-Offset 0 neu beginnen. Während des Spiels gibt es weder GPU→CPU-Readbacks noch einen periodischen Terrain-RGB-Sampler: Blätter klassifizieren ihre Layoutposition nur etwa alle 32 Weltpixel als Grass, Dirt oder neutral und wählen daraus eine gehaltene Naturpalette; Staub liest die sichtbare Snapshot-Farbe ausschließlich beim jeweiligen Spawn und behält diesen Tint.
...
Die Mündungsposition wird zentral über getTopDownMuzzleOrigin() beziehungsweise getTopDownMuzzleOriginFromVector() aus src/config.ts berechnet. MuzzleFlashRenderer erhält diese Position; nur die asymmetrischen GPU-Body-Member erhalten daraus einen texturlokalen Vorwärtsoffset, während Sparks am Originalursprung bleiben. Trails und Anhänge leiten lokale Offsets aus der normalisierten Flug-/Aimrichtung ab.
...
aus. Die Analyse bleibt dabei bewusst auf dem festen 4x4-Source-Pixel-Raster; erst
`CombatGoreGpuRenderer` skaliert die Chunk-Breite/-Hoehe mit `sourceWidth`/`sourceHeight` und
`DEATH_DISINTEGRATION_VFX.referenceDisplaySizePx` in World-/Display-Space. Die normierten Chunk-
Offsets bleiben unveraendert, damit Farbmittelung, Silhouettenabdeckung, Sampling-Reihenfolge und
Cache-Schluessel stabil bleiben. Death-Visualdaten muessen deshalb vor dem Entity-Destroy erfasst
werden; Fragmentspezifische Netzwerkdaten werden nicht repliziert.
...
Summe 68 096 vorgehaltene Member; der Instance-Buffer belegt damit rund 11,4 MB (Stride zur Laufzeit ueber `layer.getDataByteSize()`, aktuell 42 Words). Der Atlas ist 512×512 RGBA (1 MB) und traegt alle Motive; die Feuerfamilien teilen sich die vorhandenen Flame-Frames in beiden Stilen, dazu kommen `ground-fire-smoke`, `leaf-blower-dust`, die beiden Jet-Motive `flame-billow` und `flame-tongue`, die drei Muzzle-Flash-Motive sowie die Death-/Blood-Motive des gemeinsamen Gore-Controllers. `MuzzleFlashRenderer` ist eventgetrieben: pro Schuss spawnt er zwei kritische Body-Member und die quality-skalierten Sparks direkt auf `muzzle-flash`, waehrend nur der Lighting-Pulse CPU-seitig bleibt. Die Muzzle-Alpha nutzt dabei nur lokal einen Tageslicht-Boden von 0.7 bis zum globalen Emissive-Faktor; die gemeinsame Emissive-Semantik bleibt unveraendert. Die Kapazitaeten der zusammengelegten Lanes sind bewusst die Summen der fruehreren Einzelpools; reduziert wird erst gegen gemessenes `peakLive`/`capacityDrops` aus dem Performance-Export, nicht gegen die Schaetzung. `entity-burn` ist dabei die erste Obergrenze ueberhaupt fuer diesen Effekt – vorher hatte jede brennende Entity drei eigene, unbegrenzte Emitter.
...
- **Ein Motiv, das verschmelzen soll, darf keine harte Silhouette haben.** Eine steile Flanke mit definiertem Nullpunkt gibt ihm eine Kontur, und die bleibt auch additiv als Einzelkoerper erkennbar: eine zur Spitze verjuengte Flammenzunge las sich als Stapel harter Keile, die Flaeche wirkte zerrissen und hektisch. Die Flammenschichten des Bodenfeuers teilen sich deshalb eine Familie aus drei weich auslaufenden, asymmetrischen Flaechenvarianten; das breite Glutbett nutzt zwei eigene Varianten. Die Auswahl erfolgt pro Spawn seed-deterministisch und verbindet sich mit Rotation und Streckung, damit keine Kreisreihe und kein wiederkehrendes Muster sichtbar wird.
- **Das Sicherheitsfenster am Frame-Rand darf keine Form vorgeben.** Ein radialer Rand-Falloff zeichnet trotz organischer Innenstruktur erneut einen Kreis. GroundFire blendet deshalb nur gegen die vier Framekanten aus; Ausbuchtungen, Einschnitte und Asymmetrie stammen allein aus der Motivfunktion. Der ausserste Rand bleibt dabei exakt transparent.
