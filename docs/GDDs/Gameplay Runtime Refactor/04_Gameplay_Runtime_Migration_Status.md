...
- **Aktive Teilphase:** `8C – Shotgun/generische Weapon Reactions` (nächster Schritt)
- **Zuletzt abgeschlossen:** `8B – Negev Behavior` ✅
...
- **Letzter verifizierter Repository-Stand:** Teilphase 8B – Negev Behavior (dieser Commit)
- **Letzter grüner automatisierter 8B-Gate:** Fokussierter Lauf mit **5 Dateien / 42 Tests grün**; danach zweiter `npm run check` mit **352 Dateien / 2974 Tests grün**, 15 übersprungen, inklusive TypeScript-Check und grünem Vite-Build.
- **Bekannte Umgebungsflakiness:** Der erste vollständige 8B-Check hatte einen isoliert nicht reproduzierbaren `HostPhysicsSystemAllocation.test.ts`-Fehler; der isolierte Testlauf und der unmittelbar wiederholte vollständige Check waren grün. Keine 8B-Regression offen.
...
|---|---|---|---|
| `world/WorldPlayerGameplayRuntime.ts` | Owner: baut `systems`, `bindLoadout`, `destroy`-Teardown, `configureResource/Burrow`; **seit 2A einziger Lifecycle-Writer** der Player-Children über `PlayerGameplayLifecyclePort`; seit 7C materialisiert er den vollständigen `PlayerUltimateBehaviorRuntime` und bindet dessen Activation-/Modifier-Reads sowie die world-composed Gauss-Capability; seit 8A materialisiert und lifecycle-bindet er den `Ak47BehaviorRuntime`; seit 8B materialisiert und lifecycle-bindet er den `NegevBehaviorRuntime` | Owner/Lifecycle | 2A ✅ / 7B ✅ / 7C ✅ / 8A ✅ / 8B ✅ |
...
| `world/WorldCombatGameplayBinding.ts` | konsumiert `WorldPlayerGameplaySystems`-Typ + `getPlayerSystems()?.{loadout,ak47Behavior,negevBehavior,playerModifier,burrow}`; bindet AK47-Hit-/Projectile-Resolve-Outcomes und seit 8B das semantische Negev-Kill-Outcome an die jeweiligen Behavior-Owner | Combat integration | 8A ✅ / 8B ✅ / 11A / 11B |

### `LoadoutManager` (direkte Typ-Consumer, nach Consumer-Art)

| Consumer-Art | Consumer (Datei → API) |
|---|---|
| Owner/Lifecycle | `WorldPlayerGameplayRuntime` (`createLoadoutManager`, `bindLoadout`, `destroy` löst alle Setter), `ArenaWorldPlayerComposition` (reicht `createLoadoutManager` und die Gauss-Capability), `ArenaLifecycleCoordinator` (`assignDefaultLoadout`, `syncSelectedLoadout`, `removePlayer`); `PlayerUltimateBehaviorRuntime` besitzt seit 7C den gesamten Player-Ultimate-Activation-State, `Ak47BehaviorRuntime` seit 8A den gesamten AK47-Combat-State und `NegevBehaviorRuntime` seit 8B den gesamten Negev-Killstreak-State; alle werden beim Player-/World-Teardown über die World-Runtime bereinigt |
...
Aktuell keine offenen Implementierungsübergänge innerhalb der abgeschlossenen Phasen. 2A/2B/3A/3B/4A/4B/4C/5/6A/6B/7A/7B/7C/8A/8B bleiben abgeschlossen, und die nächste offene fachliche Transition ist 8C.
...
**Teilphase 8C umsetzen – Shotgun/generische Weapon Reactions.**

Dabei:

1. Shotgun-Lightning-Killqueue, Ketten-Generation und Abschlussreaktion aus `LoadoutManager` in einen eng begrenzten Behavior-/Reaction-Owner verschieben.
2. `LoadoutManager` auf verbleibende Equipment-/Shot-Delegation reduzieren; keinen Universal-Combat-Manager einführen.
3. Kill-Outcome, Queue-Verarbeitung, Damage-/Broadcast-Reaktion und Reset mit bestehender Identity-/Authority-Semantik erhalten.
4. Referenz: Phase-8C-Abschnitt in `03`.
5. Gate: Shotgun-Lightning-, Queue-/Chain-, Kill-Reaktions-, Teardown- und TypeScript-Checks.
