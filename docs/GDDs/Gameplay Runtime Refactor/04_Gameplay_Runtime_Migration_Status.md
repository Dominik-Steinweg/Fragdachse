...
| 8B | ✅ | `Phase-8B-Commit (dieser Commit)` | Negev Behavior |
| 8C | ✅ | `Phase-8C-Commit (dieser Commit)` | Shotgun/generische Weapon Reactions |
| 9 | ✅ | `Phase-9-Commit (dieser Commit)` | Tesla Dome / Energy Shield Behavior |
| 10A | ✅ | `Phase-10A-Commit (dieser Commit)` | LoadoutManager final reduzieren |
| 10B | ✅ | `Phase-10B-Commit (dieser Commit)` | NetworkBridge aus Loadout/Ability-Core |
...
|---|---|---|---|
| `world/WorldPlayerGameplayRuntime.ts` | Owner: baut `systems`, `bindLoadout`, `destroy`-Teardown, `configureResource/Burrow`; **seit 2A einziger Lifecycle-Writer** der Player-Children über `PlayerGameplayLifecyclePort`; seit 7C materialisiert er den vollständigen `PlayerUltimateBehaviorRuntime` und bindet dessen Activation-/Modifier-Reads sowie die world-composed Gauss-Capability; seit 8A materialisiert und lifecycle-bindet er den `Ak47BehaviorRuntime`; seit 8B materialisiert und lifecycle-bindet er den `NegevBehaviorRuntime`; seit 8C materialisiert und lifecycle-bindet er den `WeaponReactionRuntime` | Owner/Lifecycle | 2A ✅ / 7B ✅ / 7C ✅ / 8A ✅ / 8B ✅ / 8C ✅ |
...
| `scenes/arena/HostUpdateCoordinator.ts` | `playerSystems` = `getPlayerGameplayRuntime()?.systems`; taktet `resource.regenTick`, `burrow.update`, `loadout.update`, `weaponReaction.update()`, `negevBehavior.update(now)`, `ultimateBehavior.update(delta, now)`, `tunnel.update`, `weaponUpgrade`, `ak47StrategicTarget`, `guardianSpirit`, `repairDrone`, `slimeTrail`, `flamethrowerUpgrade`, `itemRuntime`; Active-Ultimate-/Rage-Tint-/Net-Reads und Gauss-Charge-Reads kommen aus `ultimateBehavior`, AK47-HUD-Buffs und Fire-Superiority-Verfügbarkeit seit 8A aus `ak47Behavior`, Negev-HUD-Buffs seit 8B aus `negevBehavior`, die verbleibenden Loadout-Reads bleiben Equipment-/spätere Behavior-Reads | Host-Frame + Read | 7B ✅ / 7C ✅ / 8A ✅ / 8B ✅ / 8C ✅ / 12A |
...
| `world/WorldCombatGameplayBinding.ts` | konsumiert `WorldPlayerGameplaySystems`-Typ + `getPlayerSystems()?.{loadout,ak47Behavior,negevBehavior,weaponReaction,playerModifier,burrow}`; bindet AK47-Hit-/Projectile-Resolve-Outcomes, das semantische Negev-Kill-Outcome und seit 8C generische Weapon-Kill-Reaktionen an die jeweiligen Owner | Combat integration | 8A ✅ / 8B ✅ / 8C ✅ / 11A / 11B |

### `LoadoutManager` (direkte Typ-Consumer, nach Consumer-Art)

| Consumer-Art | Consumer (Datei → API) |
|---|---|
| Owner/Lifecycle | `WorldPlayerGameplayRuntime` (`createLoadoutManager`, `bindLoadout`, `destroy` löst alle Setter), `ArenaWorldPlayerComposition` (reicht `createLoadoutManager` und die Gauss-Capability), `ArenaLifecycleCoordinator` (`assignDefaultLoadout`, `syncSelectedLoadout`, `removePlayer`); `PlayerUltimateBehaviorRuntime` besitzt seit 7C den gesamten Player-Ultimate-Activation-State, `Ak47BehaviorRuntime` seit 8A den gesamten AK47-Combat-State, `NegevBehaviorRuntime` seit 8B den gesamten Negev-Killstreak-State und `WeaponReactionRuntime` seit 8C Queue-/einfache Weapon-Reaction-State; alle werden beim Player-/World-Teardown über die World-Runtime bereinigt |
...
| Read/Presentation | `HostUpdateCoordinator`, `ClientUpdateCoordinator` (`getEquipped{Weapon,Utility,Ultimate}Config`, `ak47Behavior.getHudBuffs`, `ak47Behavior.isFireSuperiorityAvailable`, `negevBehavior.getHudBuffs`, `getShieldBuffHudState`, `getCooldownFrac`, `getAimNetState`, `getHeldItemSlot`, Rage-/Threshold-Reads), `PlayerUltimateBehaviorRuntime` (`isUltimateActive`, `getActiveUltimateId`, Gauss-Charge-Reads, Speed-/Damage-Modifier), `ArenaRuntimeAdapters` (`getEquippedUtilityConfig`, `getTemporaryUtilityConfig` über die Player-Read-View), Temporary-Utility-Projektion aus `PlayerUtilityActionRuntime` | Read/Presentation | 7A ✅ / 7B ✅ / 7C ✅ / 8A ✅ / 8B ✅ / 12A / 12B |
| Combat integration | `CombatSystem` (`setLoadoutManager` → generische Weapon-/Damage-Reads, `setAk47Behavior` → semantische AK47-Hit-/Reset-Outcomes), `WorldCombatGameplayBinding` (`weaponReaction.registerKill`, `negevBehavior.registerKill`, `getEquippedWeaponConfig`, `getDamageMultiplier`, `getSpeedMultiplier`, `Ak47BehaviorRuntime.resolveProjectile` via `projectileManager.setProjectileResolvedCallback`), `HostPhysicsSystem` (`setLoadoutManager`), `Ak47StrategicTargetSystem` (`Ak47LoadoutReadPort`), `FlamethrowerUpgradeSystem` (`resolveUtilityConfig`, `getEquippedWeaponConfig`) | Combat integration | 8A ✅ / 8B ✅ / 8C ✅ / 11A / 11B |
...
| network adapter (Legacy) | `WorldPlayerGameplayRuntime` erhält fachliche `PlayerRelationshipPort`-/Presentation-/Projection-Ports; `LoadoutManager.ts` ist ohne konkreten `NetworkBridge`. Utility-Cooldown-, Temporary-Utility-, Held-Utility- und Shot-FX-Publikation liegen an den jeweiligen World-/Behavior-/Composition-Grenzen. | network adapter (Legacy) | 7A / 10B ✅ |
...
`LoadoutManager.use` und `WorldPlayerGameplayRuntime.useLegacyLoadoutAction` sind entfernt. Der produktive Player-Waffenpfad ist `ArenaRuntimeAdapters` → `PlayerLoadoutRpcPort.usePlayerAction` → `WorldPlayerGameplayRuntime.usePlayerAction` → `PlayerActionRuntime` → `PlayerWeaponActivationRuntime`; Charakterisierungs- und Slot-Tests verwenden den semantischen Zielpfad bzw. die neue Aktivierungsgrenze direkt. `GameplayRuntimeCorrectionRatchets.test.ts` friert das Fehlen des alten Dispatches ein. Utility und Ultimate committen ausschließlich über ihre benannten Runtime-Owner; Airstrike, Tunnel und Gauss behalten getrennte Support-/Construction-/Execution-Capabilities. Die NetworkBridge bleibt für Weapon2-Prediction-Dedupe und Prediction-ID zuständig.
...
- Negev: `NegevBehaviorRuntime` ist seit 8B der world-owned Single-Writer für Killstreak-Kills, erfolgreichen Schusszeitpunkt, Streak-Gap/Termination, Schussmodifikator, Kill-Heal/-Armor, Abschluss-AoE/-Knockback, Fire-Chunk-Reaktion und HUD. `WorldCombatGameplayBinding` meldet `NegevKillOutcome`; `LoadoutManager` delegiert Shot-Vorbereitung/Commit und Resource-Reject-Termination; Attach/Detach/Reconcile/Destroy bereinigen den Behavior-State. Phase 8B ✅.
- Shotgun/einfache Weapon-Reaktionen: `WeaponReactionRuntime` ist seit 8C der world-owned Single-Writer für die Shotgun-Lightning-Queue, Chain-Generation und config-getriebene Kill-Heal-/Adrenalin-Reaktionen. `WorldCombatGameplayBinding` meldet das vollständige `WeaponKillReactionOutcome`; `HostUpdateCoordinator` taktet die Queue; `LoadoutManager` enthält weder `handleKill` noch mutable Reaction-State. Phase 8C ✅.
...
| Spätere Fachphase | Verbleibende Legacy-Zeitstelle im Stand `Phase 9` |
|---|---|
...
| **8B – Negev Behavior** | erledigt: `NegevBehaviorRuntime` erhält explizite Host-Zeit für erfolgreichen Shot-Commit, Streak-Gap und Abschlussreaktion; `LoadoutManager` enthält keinen Negev-Killstreak-State und keinen `Date.now()`-Default für diese Semantik. |
| **9 – Tesla Dome / Energy Shield Behavior** | `LoadoutManager.getShieldBuffHudState` besitzt noch einen `Date.now()`-Default; dieser Read bleibt bis zur geplanten Combat-/Frame-Read-Bereinigung zurückgestellt. Der Shield-/Dome-Anteil von `getSpeedMultiplier` und die dazugehörigen zeitabhängigen Reads liegen jetzt im `SustainedWeaponBehaviorRuntime`. |
...
| `WorldGameplayCompositionContracts.test.ts` | eingefrorene `NetworkBridge`-Consumer-Liste ohne `src/loadout/LoadoutManager.ts`; neue World-Owner frei von `NetworkBridge`/`ArenaContext`; `PlayerRelationshipPort` wird an der Composition-Grenze gebunden; `new WorldPlayerGameplayRuntime` nur in Composition. **4A: `new WorldWeaponExecutionRuntime` + `worldRuntime.bind(weaponExecution)` in die Owner-/Bind-Liste aufgenommen** | 2A / 4A ✅ / 10B ✅ |
...
| Weapon-Slot-Exklusivität / Channel-Switch-Deaktivierung | `WeaponSlotExclusivity.test.ts`, `SustainedWeaponBehaviorRuntime.test.ts` | abgedeckt (Behavior besitzt Slot-Claim, Channel-Stop und Autonomous-Toggle-Ausnahme) | geprüft bei 9 ✅ |
...
| **Negev-Killstreak-Runtime: Kill-Zahl, Streak-Gap-Ende in `update()`, Abschlussexplosion** | `NegevBehaviorRuntime.test.ts`, `WorldCombatGameplayBinding.test.ts`, `WorldPlayerGameplayLifecycle.test.ts`, `GameplayRuntimeCorrectionRatchets.test.ts` | abgedeckt (8B: world-owned State, expliziter Shot-Commit-/Gap-Zeitwert, Kill-Reaktion, Abschluss-AoE/-Knockback/Fire-Chunk, HUD und Teardown) | geprüft bei 8B ✅ |
| **Shotgun-Lightning: Kill → Queue → `applyAoeDamage` + Broadcast** | `WeaponReactionRuntime.test.ts`, `WorldCombatGameplayBinding.test.ts`, `WorldPlayerGameplayLifecycle.test.ts`, `GameplayRuntimeCorrectionRatchets.test.ts` | abgedeckt (8C: world-owned Queue, Chain-Generation/Retention, Slow-Payload, Combat-/Effect-Reaktion und Teardown) | geprüft bei 8C ✅ |
| **Config-getriebene Kill-Reaktionen: Heal/Adrenalin** | `WeaponReactionRuntime.test.ts`, `WorldCombatGameplayBinding.test.ts` | abgedeckt (8C: nur ausgerüstete Waffenquelle, bestehende Config-Werte und semantischer Kill-Callback) | geprüft bei 8C ✅ |
...
| Tesla Dome / Energy Shield start/refresh/stop-Orchestrierung | `TeslaDomeSystem.test.ts`, `TeslaDomeCoopDefenseUpgrades.test.ts`, `WeaponSlotExclusivity.test.ts`, `SustainedWeaponBehaviorRuntime.test.ts` | abgedeckt (bestehende Effekt-Owner plus world-owned Sustained-Behavior mit Resource-Gate, Refresh, Channel-/Toggle-Semantik und Teardown) | geprüft bei 9 ✅ |
...
Aktuell keine offenen Implementierungsübergänge innerhalb der abgeschlossenen Phasen. 2A/2B/3A/3B/4A/4B/4C/5/6A/6B/7A/7B/7C/8A/8B/8C/9/10A/10B bleiben abgeschlossen, und die nächste offene fachliche Transition ist 11A.
...
- **Equipment- und Dispatch-Korrekturen:** Ein tatsächlicher Utility-Wechsel übernimmt keinen alten Cooldown; ein No-op-Sync publiziert keine unnötige Projektion. Der alte slotförmige Weapon-Dispatch ist entfernt und durch den semantischen `PlayerActionRuntime` → `PlayerWeaponActivationRuntime`-Pfad ersetzt.
...
- **8B** verschiebt den gesamten veränderlichen Negev-Killstreak-State aus `LoadoutManager` in `NegevBehaviorRuntime`. Der Behavior-Owner führt erfolgreiche Shot-Zeit, Killstreak, Gap-Termination, Damage-/Burn-Skalierung, Kill-Heal/-Armor, Abschluss-AoE/-Knockback, Fire-Chunk-Anforderung und HUD-Projektion; `WorldCombatGameplayBinding` meldet ein typed `NegevKillOutcome`, während `LoadoutManager` nur Equipment-/Shot-Delegation und Reject-Termination behält. Attach/Detach/Reconcile/Destroy bleiben über `WorldPlayerGameplayRuntime` lifecycle-gebunden.
- **8C** verschiebt Shotgun-Lightning und einfache config-getriebene Kill-Reaktionen aus `LoadoutManager` in `WeaponReactionRuntime`. Der world-owned Reaction-Owner hält Queue und Chain-Generation, führt bestehende AoE-/Slow-/Broadcast-Semantik aus und behandelt Kill-Heal/-Adrenalin nur für die ausgerüstete Quellenkonfiguration; `WorldCombatGameplayBinding` meldet ein vollständiges typed `WeaponKillReactionOutcome`, und Host-Frame/Player-Lifecycle binden Update/Reset/Remove/Destroy.
- **9** verschiebt Slot-Claim-/Switch-Semantik sowie Start/Refresh, Adrenalin-Gate, Stop und Movement-Reads für Tesla Dome und Energy Shield in den world-owned `SustainedWeaponBehaviorRuntime`. `TeslaDomeSystem` und `EnergyShieldSystem` bleiben die fachlichen Effekt-State-Owner; Player-Loadout-Attach/Detach/Reconcile/Destroy und die World-Combat-Bindings führen die Behavior-Lifetime. Autonome Energy-Dome-Toggles überleben weiterhin einen Weapon-Switch, nicht-autonome Kanäle werden sofort beendet.
- **10A** reduziert `LoadoutManager` auf Equipment-/Selection-State, effektive Config-Auflösung, item-lokale Weapon-Readiness/Spread, held-item-/Aim-/Presentation-Reads und Utility-/Ultimate-Read-Ports. `PlayerWeaponActivationRuntime` besitzt nun die unmittelbare Weapon1/Weapon2-Aktivierung, Ressourcenprüfung/-Commit, AK47-/Negev-Vorbereitung, Kinetik-Item-Verbrauch, Side-Burst, Recoil, Screenshake und Capability-Dispatch; `PlayerActionRuntime` bleibt dessen einzige Player-Action-Zuleitung. Die Loadout-Selektion nutzt bis 10B noch den schmalen Mode-Adapter; der NetworkBridge-Cutover ist die nächste Teilphase.
- **10B** entfernt `NetworkBridge` aus `LoadoutManager` und den neuen Player-Action-/Behavior-Ownern. Selection/Sanitization erhält einen kleinen `LoadoutSelectionModePort`; Relationship-Semantik läuft über `PlayerRelationshipPort`, dessen Composition-Adapter vorläufig `NetworkBridge.isEnemyPair` delegiert. Utility-, Temporary-Utility-, Held-Item- und Shot-FX-Publikation bleiben an den jeweiligen fachlichen World-/Behavior-/Composition-Grenzen; die eingefrorene Bridge-Consumer-Liste schrumpft um `LoadoutManager` und wächst nicht.
...
| `PlayerWeaponActivationRuntime` | `class PlayerWeaponActivationRuntime` in `src/world/PlayerWeaponActivationRuntime.ts`; besitzt die unmittelbare Weapon1/Weapon2-Aktivierung, hostzeitgebundene Ressourcenprüfung/-buchung, AK47-/Negev-Shot-Vorbereitung und -Commit, Kinetik-Ladungsverbrauch, Spread-/Cooldown-Commit-Reihenfolge, Side-Burst, Recoil, Screenshake und den Dispatch an world-composed Execution-Capabilities. Der Runtime-Owner hält keinen Equipment-State; Readiness/Spread und held-item-Projektion bleiben im `LoadoutManager`, Behavior-State in den jeweiligen Behavior-Ownern. | 10A |
...
| `SustainedWeaponBehaviorPort` | `interface SustainedWeaponBehaviorPort`, `SustainedWeaponLoadoutReadPort` und `SustainedWeaponActionRequest` in `src/loadout/SustainedWeaponBehaviorPort.ts`; `SustainedWeaponBehaviorRuntime` in `src/world/SustainedWeaponBehaviorRuntime.ts` besitzt Tesla-/Energy-Slot-Claim, Channel-/Autonomous-Switch-Regel, Start/Refresh, Resource-Gate, Movement-Read sowie Reset/Remove/Destroy. `LoadoutManager` konsumiert nur die kleine Behavior-Grenze; `TeslaDomeSystem` und `EnergyShieldSystem` bleiben Effekt-State-Owner. | 9 |
...
| `PlayerRelationshipPort` | `interface PlayerRelationshipPort` in `src/world/PlayerRelationshipPort.ts`; World-Player-Gameplay-Owner konsumieren Relationship als Domain-Read statt als `network.teams`-Semantik. Der Composition-Adapter delegiert vorläufig an `NetworkBridge.isEnemyPair`, ohne Relationship-Authority in den Transport zu verlagern. | 10B |
...
**Teilphase 11A umsetzen – PlayerCombatIntegration: Reads, Modifier und Resource-Semantik.**

Dabei:

1. Die Read-/Modifier-Seite des Combat-Bindings über semantische Player-Gameplay-Ports führen.
...
4. Referenz: Phase-11A-Abschnitt in `03`.
5. Gate: `WorldCombatGameplayBinding.test.ts`, Modifier-/Resource-Integration, World-Composition-Contracts und TypeScript-Check.
