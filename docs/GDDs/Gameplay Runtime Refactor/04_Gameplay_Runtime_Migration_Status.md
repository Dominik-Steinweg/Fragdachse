...
- **Aktive Teilphase:** `7C – Airstrike/Tunnel/Gauss Ultimate` (nächster Schritt)
- **Zuletzt abgeschlossen:** `7B – Buff-/Armageddon-Ultimate Behavior` ✅
- **Gesamtstatus:** Lifecycle-, Read-View-, Action-Zeit-, Resource-/Readiness-Zeit-Grenzen und die world-composed Immediate-Weapon-Execution-Capabilities stehen. Construction-/Management-Readiness liegt im World-Construction-Owner und wird über einen schmalen Port von der Persistent-Base-Session genutzt; Konstruktion, Einzel-Rückbau und Persistent-Base-Repositionierung verwenden explizite Host-Zeit. Gemeinsame Projectile/Hitscan/Melee-Fälle laufen über `WeaponExecutionCapability`; Flamethrower, Leaf Blower, Reinforcement Matrix und Energy Injector über die benannte `SpecializedWeaponExecutionCapability`. Der World-Player-Owner besitzt mit `PlayerActionRuntime` die hostautoritative Weapon1/Weapon2-Action-Grenze, seit 6B die Held-Action-, Burrow- und Resource-Command-Grenze, seit 7A die semantische Utility-Action-Grenze und seit 7B die Buff-/Armageddon-Ultimate-Behavior-Grenze. `PlayerUtilityActionRuntime` ist der Single-Writer für Equipped-Utility-Cooldown/Commit, Temporary-Utility-Identity/Charges und die Utility-Routen; `TemporaryUtilityCollection` bleibt State-Owner für temporäre Instanzen. `PlayerUltimateBehaviorRuntime` besitzt Buff-/Aura-/Armageddon-Lifecycle, Rage-Drain, Ticks, Linger und die zugehörigen Active-/Modifier-Reads; `WorldSupportGameplayRuntime` stellt dafür nur die world-scoped Armageddon-Capability bereit. Objective-Rewards nutzen einen schmalen `TemporaryUtilityPort`, Construction erhält nur die Placement-Capability. Buff-/Armageddon-Aktivierung läuft nicht mehr über `LoadoutManager`; Airstrike, Tunnel und Gauss bleiben bis 7C im benannten Legacy-Pfad. Gegner, Türme, Necromancy und Void-Hunter-Gauss behalten ihre automatische Boundary; Timing, Readiness und Autorität bleiben bei ihren jeweiligen Ownern.
- **Letzter verifizierter Repository-Stand:** Phase-7B-Commit (dieser Commit)
- **Letzter grüner automatisierter Phase-Gate:** Fokussierter 7B-Lauf mit **4 Dateien / 35 Tests grün** (`PlayerUltimateBehaviorRuntime`, Ultimate-Statistik, Player-Lifecycle und Charakterisierung); danach `npm run check` mit **350 Dateien / 2957 Tests grün**, 15 übersprungen, sowie grünem `tsc + vite build`.
- **Bekannte Umgebungsflakiness (nicht durch dieses Refactoring verursacht):** `tests/WorldCombatGameplayBinding.test.ts` AK47-„random visible target\" bleibt wegen `Math.random` grundsätzlich flakiness-anfällig; im vollständigen 7A-Gate trat sie nicht auf. Die vier `ArenaTransitionReadiness`-Source-Scans und TypeScript-/Build-Gates waren im aktuellen Checkout grün.
...
| 7A | ✅ | `Phase-7A-Commit` | Utility Activation + Temporary Utilities |
| 7B | ✅ | `Phase-7B-Commit` | Buff-/Armageddon-Ultimate Behavior |
...
| `scenes/arena/HostUpdateCoordinator.ts` | `playerSystems` = `getPlayerGameplayRuntime()?.systems`; taktet `resource.regenTick`, `burrow.update`, `loadout.update`, seit 7B `ultimateBehavior.update(delta, now)`, `tunnel.update`, `weaponUpgrade`, `ak47StrategicTarget`, `guardianSpirit`, `repairDrone`, `slimeTrail`, `flamethrowerUpgrade`, `itemRuntime`; Active-Ultimate-/Rage-Tint-/Net-Reads kommen seit 7B aus `ultimateBehavior`, die verbleibenden Loadout-Reads bleiben für Equipment bzw. 7C | Host-Frame + Read | 7B ✅ / 12A |
...
| `scenes/arena/ArenaWorldConstructionComposition.ts` | `gameplay.player?.systems.{loadout,playerModifier,tunnel,burrow,resource}` für Construction-internes Composition-Wiring; Temporary-Utility-Akquisition über `gameplay.player?.addTemporaryUtility`, Placement über die schmale Utility-Capability | Construction-Composition | 5 / 7A ✅ |
...
|---|---|
| Owner/Lifecycle | `WorldPlayerGameplayRuntime` (`createLoadoutManager`, `bindLoadout`, `destroy` löst alle Setter), `ArenaWorldPlayerComposition` (reicht `createLoadoutManager`), `ArenaLifecycleCoordinator` (`assignDefaultLoadout`, `syncSelectedLoadout`, `removePlayer`); `PlayerUltimateBehaviorRuntime` besitzt seit 7B den Buff-/Armageddon-Lifecycle und wird beim Player-/World-Teardown zuerst bereinigt |
| Action | Weapon1/Weapon2: `ArenaRuntimeAdapters` → `PlayerLoadoutRpcPort.usePlayerAction` → `WorldPlayerGameplayRuntime.usePlayerAction` → `PlayerActionRuntime`; Equipped Utility, Temporary Utility und Inspector-Utility: derselbe Player-Action-Port → `PlayerUtilityActionRuntime` mit getrennter Source-Identity; Construction bleibt der eigene Use-Case. Ultimate: benannter Legacy-Pfad `useLegacyLoadoutAction` → `LoadoutManager.use`. RPC bestimmt (3A/6B) einen einzigen `hostNowMs`; Charged Utilities validieren Held-Action-Identity und committen at-most-once, Charges/Cooldown werden erst nach erfolgreichem Utility-Dispatch geschrieben. Weapon2-Antworten werden weiterhin um `worldRevision`/`authoritativeAdrenaline`/`adrenalineRevision` augmentiert; Held Actions, Burrow und Resource-Tooling mutieren ausschließlich über `WorldPlayerGameplayRuntime`-Commands. |
| Read/Presentation | `HostUpdateCoordinator`, `ClientUpdateCoordinator` (`getEquipped{Weapon,Utility,Ultimate}Config`, `getAk47HudBuffs`, `getNegevHudBuffs`, `getShieldBuffHudState`, `getCooldownFrac`, `getAimNetState`, `getHeldItemSlot`, `getUltimate*`), `PlayerUltimateBehaviorRuntime` (`isUltimateActive`, `getActiveUltimateId`, Speed-/Damage-Modifier), `ArenaRuntimeAdapters` (`getEquippedUtilityConfig`, `getTemporaryUtilityConfig` über die Player-Read-View), Temporary-Utility-Projektion aus `PlayerUtilityActionRuntime` | Read/Presentation | 7A ✅ / 7B ✅ / 12A / 12B |
| Combat integration | `CombatSystem` (`setLoadoutManager` → Weapon-/Damage-Reads), `WorldCombatGameplayBinding` (`handleKill`, `getEquippedWeaponConfig`, `getDamageMultiplier`, `getSpeedMultiplier`, `resolveAk47Projectile` via `projectileManager.setProjectileResolvedCallback`), `HostPhysicsSystem` (`setLoadoutManager`), `Ak47StrategicTargetSystem` (`getEquippedWeaponConfig`), `FlamethrowerUpgradeSystem` (`resolveUtilityConfig`, `getEquippedWeaponConfig`) | Combat integration | 11A / 11B |
| Construction / Persistent Base | `ConstructionWorldRuntime` (`useInspectorUtility`, Construction-Handler und `ConstructionReadinessPort`); Inspector-Utility wird an `PlayerUtilityActionRuntime` delegiert, Construction-/Persistent-Base-Rechte und Readiness bleiben im Construction-Owner; `ArenaWorldConstructionComposition` nutzt den `TemporaryUtilityPort` | Construction / Persistent Base | 5 / 7A ✅ |
...
| Support-/Ultimate-Handler-Wiring | `WorldSupportGameplayRuntime` stellt seit 7B `systems.armageddon` als Capability bereit; `ArenaWorldEnvironmentComposition` bindet sie an `PlayerUltimateBehaviorRuntime`. `LoadoutManager` wird für `setAirstrikeHandler` bis 7C weiterhin genutzt; Utility-Dispatch-Hooks für Translocator, Decoy, StinkCloud, Nuke und Placement sind entfernt und liegen bei den jeweiligen World-/Behavior-Ownern. | Support-/Ultimate-Handler-Wiring | 7A ✅ / 7B ✅ / 7C |
...
### `LoadoutManager.use` – verbleibender Legacy-Call-Site für Nicht-Buff-Ultimates

`ArenaRuntimeAdapters` → `PlayerLoadoutRpcPort.useLoadout` → `RpcCoordinator.registerLoadoutUseHandler` bleibt für Airstrike, Tunnel und Gauss samt ihrer Sonderpfade bestehen. Ein Buff-/Armageddon-Use wird im World-Owner aus diesem Legacy-Kompatibilitätseinstieg in `PlayerUltimateBehaviorRuntime.execute` umgeleitet; `LoadoutManager.use` weist den Buff-Fall als ungültigen Zweitpfad zurück. Equipped Utility, Temporary Utility und Inspector-Utility laufen über `PlayerLoadoutRpcPort.usePlayerAction` in `PlayerUtilityActionRuntime`; Construction bleibt über `ConstructionWorldRuntime` und seine Placement-Capability getrennt. Weapon1/Weapon2 gehen ebenfalls über `PlayerActionRuntime`; der interne Utility-/Held-State-Owner bleibt jeweils eindeutig. Die NetworkBridge bleibt für Weapon2-Prediction-Dedupe und Prediction-ID zuständig.
...
| Spätere Fachphase | Verbleibende Legacy-Zeitstelle im Stand `Phase 7A` |
|---|---|
...
| Airstrike-/Tunnel-Ultimate Accept-Reject + Rage-Kosten-Zeitpunkt | – | **Lücke** (Gauss bleibt separat abgedeckt) | in 7C konkretisieren, dort Test ergänzen |
...
Aktuell keine offenen Implementierungsübergänge innerhalb der abgeschlossenen Phasen. 2A/2B/3A/3B/4A/4B/4C/5/6A/6B/7A/7B sind abgeschlossen; die nächste offene fachliche Transition ist 7C.
...
- **7B** verschiebt Buff-/Armageddon-Ultimates aus `LoadoutManager` in den World-owned `PlayerUltimateBehaviorRuntime`. Dessen State-Map ist der einzige Writer für Aktivierung, proportionalen Rage-Drain, Armor-/Aura-Ticks, Aura-Linger und Ende/Reset; der Behavior-Owner liest die ausgerüstete Buff-Config aus dem Loadout und liefert Speed-/Damage-Modifikatoren zurück. `WorldSupportGameplayRuntime` stellt Armageddon nur als Capability bereit; `LoadoutManager` behält bis 7C ausschließlich die Equipment- und Nicht-Buff-Ultimate-Pfade sowie den getrennten Gauss-Charge-State.
...
| `PlayerActionRequest` | `interface PlayerWeaponActionRequest`, `interface PlayerUtilityActionRequest` und `interface PlayerUltimateActionRequest` in `src/world/PlayerActionRuntime.ts`, zusammengeführt als `PlayerActionRequest`-Union mit `category: 'weapon' | 'utility' | 'ultimate'`; `PlayerActionRuntime` bleibt die Weapon-Grenze, `PlayerUtilityActionRuntime` die Utility-Grenze und `PlayerUltimateBehaviorRuntime` die Buff-Ultimate-Grenze unter demselben `WorldPlayerGameplayRuntime.usePlayerAction`-Port. Explizite `hostNowMs`-Zeit und die bisherige Client-Positionspolicy bleiben erhalten. | 6A / 7A / 7B |
...
| `PlayerUltimateBehaviorRuntime` | `class PlayerUltimateBehaviorRuntime` in `src/world/PlayerUltimateBehaviorRuntime.ts`; besitzt Buff-/Aura-/Armageddon-State, duplicate-safe Buff-Commit, Rage-Drain, Armor-/Aura-Ticks, Linger, Active-/Modifier-Reads sowie Player-/World-Teardown. `PlayerUltimateArmageddonCapability` wird aus `WorldSupportGameplayRuntime.systems.armageddon` an der World-Composition-Grenze gebunden; `LoadoutManager` konsumiert nur die read-only `UltimateModifierReadPort`. | 7B |
...
**Teilphase 7C umsetzen – Airstrike/Tunnel/Gauss Ultimate.**

Dabei:

1. Airstrike-, Tunnel- und Gauss-Ultimates über die bestehende Player-Action-Grenze adressieren und Activation-/Placement-/Charge-State getrennt von Equipment halten.
2. Den jeweiligen Ultimate-/Behavior-State-Owner und seine Player-in-World- bzw. World-Lifetime explizit bestimmen.
3. Airstrike-, Tunnel- und Gauss-Verhalten nach vorhandenen Support-/Placement-/Execution-Capabilities routen; keine neue Universal-Ability-Schicht einführen.
4. Ultimate-Activation mit hostseitiger Zeit, Accept-/Reject-Gates und bestehender Resource-/Commit-Reihenfolge migrieren.
5. Rage-/Cooldown-/Charge-State erst nach erfolgreichem Ultimate-Commit fortschreiben und bestehende Network-/HUD-Projektionen erhalten.
6. Referenz-§§: `02` §§ 3, 8.1, 14–18, 27–29; Phase-7C-Abschnitt in `03`.
7. Gate: Airstrike-/Tunnel-/Gauss-Ultimate-Tests, TypeScript-Check und die betroffenen Player-/Round-Lifecycle-Tests.
