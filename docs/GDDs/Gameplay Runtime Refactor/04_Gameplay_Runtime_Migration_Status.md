...
| 7C | ✅ | `Phase-7C-Commit` | Airstrike/Tunnel/Gauss Ultimate |
| 8A | ⬜ | — | AK47 Behavior |
| 8B | ⬜ | — | Negev Behavior |
...
| `scenes/arena/HostUpdateCoordinator.ts` | `playerSystems` = `getPlayerGameplayRuntime()?.systems`; taktet `resource.regenTick`, `burrow.update`, `loadout.update`, `ultimateBehavior.update(delta, now)`, `tunnel.update`, `weaponUpgrade`, `ak47StrategicTarget`, `guardianSpirit`, `repairDrone`, `slimeTrail`, `flamethrowerUpgrade`, `itemRuntime`; Active-Ultimate-/Rage-Tint-/Net-Reads und Gauss-Charge-Reads kommen aus `ultimateBehavior`, die verbleibenden Loadout-Reads bleiben Equipment-/spätere Behavior-Reads | Host-Frame + Read | 7B ✅ / 7C ✅ / 12A |
...
### `LoadoutManager.use` – verbleibender Legacy-Call-Site für Weapon-Kompatibilität

`PlayerLoadoutRpcPort.usePlayerAction` und `RpcCoordinator.registerLoadoutUseHandler` routen alle vier Player-Action-Kategorien direkt über `WorldPlayerGameplayRuntime`. `LoadoutManager.use` bleibt nur als slotförmige interne Waffenkompatibilität für bestehende Charakterisierungstests erhalten; Utility und Ultimate weisen diesen Zweitpfad als ungültig zurück. Airstrike, Tunnel und Gauss committen ausschließlich über `PlayerUltimateBehaviorRuntime` und ihre benannten World-Capabilities. Die NetworkBridge bleibt für Weapon2-Prediction-Dedupe und Prediction-ID zuständig.
...
| Spätere Fachphase | Verbleibende Legacy-Zeitstelle im Stand `Phase 7C` |
|---|---|
...
| Gauss-Ultimate press/release Commit (nur bei Vollladung, at-most-once) | `RoomStatisticsGameplayHooks.test.ts`, `PlayerUltimateBehaviorRuntime.test.ts` | abgedeckt (7C: Host-Zeit, Client-Fraction nicht autoritativ, Execution-Reject ohne Rage-/Charge-Verlust) | geprüft bei 7C ✅ |
...
Aktuell keine offenen Implementierungsübergänge innerhalb der abgeschlossenen Phasen. 2A/2B/3A/3B/4A/4B/4C/5/6A/6B/7A/7B/7C sind abgeschlossen; die nächste offene fachliche Transition ist 8A.
...
| `PlayerActionRequest` | `interface PlayerWeaponActionRequest`, `interface PlayerUtilityActionRequest` und `interface PlayerUltimateActionRequest` in `src/world/PlayerActionRuntime.ts`, zusammengeführt als `PlayerActionRequest`-Union mit `category: 'weapon' | 'utility' | 'ultimate'`; `PlayerActionRuntime` bleibt die Weapon-Grenze, `PlayerUtilityActionRuntime` die Utility-Grenze und `PlayerUltimateBehaviorRuntime` die vollständige Ultimate-Grenze unter demselben `WorldPlayerGameplayRuntime.usePlayerAction`-Port. Explizite `hostNowMs`-Zeit und die bisherige Client-Positionspolicy bleiben erhalten. | 6A / 7A / 7B / 7C |
...
**Teilphase 8A umsetzen – AK47 Behavior extrahieren.**

Dabei:

1. AK47-Stacks, Fire-Superiority-Salven, Shot-Identity, bestätigte Hits, Strategic-Target-Refunds, Reset und HUD-Buffs in einen eigenen Weapon-Behavior-Owner verschieben.
2. `Ak47StrategicTargetSystem` auf einen kleinen AK47-/Loadout-Read-Port umstellen.
3. Projectile-/Combat-Callbacks als semantische Outcomes an den Behavior-Owner melden; Duplicate-safe Refund-/Resolve-Semantik erhalten.
4. Keine neue Projectile-Payload-Architektur einführen.
5. Referenz: Phase-8A-Abschnitt in `03`.
6. Gate: AK47-Focus-/Fire-Superiority-, Strategic-Target-, Projectile-Hit-/Resolve- und HUD-Buff-Tests sowie TypeScript-Check.
