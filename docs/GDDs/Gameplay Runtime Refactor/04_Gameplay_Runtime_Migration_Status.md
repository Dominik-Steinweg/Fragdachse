...
- **Aktive Teilphase:** `7B – Buff-/Armageddon-Ultimate Behavior` (nächster Schritt)
- **Zuletzt abgeschlossen:** `7A – Utility Activation und Temporary Utilities` ✅
- **Gesamtstatus:** Lifecycle-, Read-View-, Action-Zeit-, Resource-/Readiness-Zeit-Grenzen und die world-composed Immediate-Weapon-Execution-Capabilities stehen. Construction-/Management-Readiness liegt im World-Construction-Owner und wird über einen schmalen Port von der Persistent-Base-Session genutzt; Konstruktion, Einzel-Rückbau und Persistent-Base-Repositionierung verwenden explizite Host-Zeit. Gemeinsame Projectile/Hitscan/Melee-Fälle laufen über `WeaponExecutionCapability`; Flamethrower, Leaf Blower, Reinforcement Matrix und Energy Injector über die benannte `SpecializedWeaponExecutionCapability`. Der World-Player-Owner besitzt mit `PlayerActionRuntime` die hostautoritative Weapon1/Weapon2-Action-Grenze, seit 6B die Held-Action-, Burrow- und Resource-Command-Grenze und seit 7A die semantische Utility-Action-Grenze. `PlayerUtilityActionRuntime` ist der Single-Writer für Equipped-Utility-Cooldown/Commit, Temporary-Utility-Identity/Charges und die Utility-Routen; `TemporaryUtilityCollection` bleibt State-Owner für temporäre Instanzen. Objective-Rewards nutzen einen schmalen `TemporaryUtilityPort`, Construction erhält nur die Placement-Capability. Utility-Aktivierung läuft nicht mehr über `LoadoutManager`; Ultimate bleibt bis 7B/7C im benannten Legacy-Pfad. Gegner, Türme, Necromancy und Void-Hunter-Gauss behalten ihre automatische Boundary; Timing, Readiness und Autorität bleiben bei ihren jeweiligen Ownern.
- **Letzter verifizierter Repository-Stand:** Phase-7A-Commit (dieser Commit)
- **Letzter grüner automatisierter Phase-Gate:** Fokussierter 7A-Lauf mit **20 Dateien / 208 Tests grün** (`TemporaryUtilityLifecycle`, Radial Input/RPC, Translocator, Decoy/StinkCloud/TimeBubble/Nuke, Construction/Objective utility und Runtime-Boundaries); danach `npm run check` mit **349 Dateien / 2954 Tests grün**, 15 übersprungen, sowie grünem `tsc + vite build`.
...
| Support-/Ultimate-Handler-Wiring | `WorldSupportGameplayRuntime` nutzt `LoadoutManager` weiterhin nur für `setArmageddonSystem`/`setAirstrikeHandler`; Utility-Dispatch-Hooks für Translocator, Decoy, StinkCloud, Nuke und Placement sind entfernt und liegen bei den jeweiligen World-/Behavior-Ownern. | Support-/Ultimate-Handler-Wiring | 7A / 7B / 7C |
...
Aktuell keine offenen Implementierungsübergänge innerhalb der abgeschlossenen Phasen. 2A/2B/3A/3B/4A/4B/4C/5/6A/6B/7A sind abgeschlossen; die nächste offene fachliche Transition ist 7B.
...
| `PlayerActionRequest` | `interface PlayerWeaponActionRequest` und `interface PlayerUtilityActionRequest` in `src/world/PlayerActionRuntime.ts`, zusammengeführt als `PlayerActionRequest`-Union mit `category: 'weapon' | 'utility'`; `PlayerActionRuntime` bleibt die Weapon-Grenze, `PlayerUtilityActionRuntime` die Utility-Grenze unter demselben `WorldPlayerGameplayRuntime.usePlayerAction`-Port. Explizite `hostNowMs`-Zeit und die bisherige Client-Positionspolicy bleiben erhalten. | 6A / 7A |
...
**Teilphase 7B umsetzen – Buff-/Armageddon-Ultimate Behavior.**

Dabei:

1. Buff-/Armageddon-Ultimates über die bestehende Player-Action-Grenze adressieren und Ultimate-Identity, Buff-State sowie Commit-Owner getrennt halten.
2. Den bestehenden Ultimate-/Buff-State-Owner und seine Player-in-World-Lifetime explizit bestimmen.
3. Buff- und Armageddon-Verhalten nach vorhandenen Ultimate-/Behavior-Capabilities routen; keine neue Universal-Ability-Schicht einführen.
4. Ultimate-Activation mit hostseitiger Zeit, Accept-/Reject-Gates und bestehender Resource-/Commit-Reihenfolge migrieren.
5. Rage-/Cooldown-State erst nach erfolgreichem Ultimate-Commit fortschreiben und bestehende Network-/HUD-Projektionen erhalten.
6. Referenz-§§: `02` §§ 3, 8.1, 14–18, 27–29; Phase-7B-Abschnitt in `03`.
7. Gate: Buff-/Armageddon-Ultimate-Tests, TypeScript-Check und die betroffenen Player-/Round-Lifecycle-Tests.
