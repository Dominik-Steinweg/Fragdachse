...
**Letzter verifizierter Repository-Stand:** `main` nach Phase-9-Auditkorrektur
**Automatisierter Gate für dieses Refactoring:** `npm run check` grün (341 Testdateien, 2880 Tests, 15 skipped; `tsc` + `vite build` erfolgreich)
...
Stand: 16 Dateien / 29 Assertion-Stellen; die Phase-6B-Ratchetts prüfen zusätzlich den neuen `ArenaRuntime`-/`WorldPresentationFrameBinding`-Pfad, Phase 8 den Top-Level-Frame-Cutover und Phase 9 die Abwesenheit der konkreten Coop-Presentation-Systemliste. Weitere Pfadschreibweisen, Glob- oder Verzeichnis-Scans auf die Scene existieren nicht (per `rg` über `readFileSync`, `readdirSync`, `src/scenes` in `tests/` verifiziert).
...
| 19 | `WorldMetricsScopeContracts` · Respawn-Kontext aus der aktiven World | B | Spawn-Map aus `world.descriptor.definitionId`, nicht aus Rundenstate | `ArenaScene:852` Spawn-Provider | `ArenaRuntime`/`WorldRuntime` | 9 |
