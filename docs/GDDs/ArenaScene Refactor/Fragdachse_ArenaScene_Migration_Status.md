...
**Aktive Phase:** `6B – Clientseitige World-Projektion verschieben`
**Gesamtstatus:** `🟨 Phase 6A.2 abgeschlossen – die allgemeine World-Darstellung (Surface/Residency, Canopy, lokale Player-/Persistent-Base-Visuals, Shadows und Lighting) hängt am world-scoped Presentation-Owner; Checkpoint A/B warten weiterhin auf manuelle Sichtprüfung, Checkpoint C folgt nach 6B`
**Letzter verifizierter Repository-Stand:** `main` nach Phase 6A.2
...
**Manueller Gate:** `offen – visuelle Prüfung nicht ausgeführt (Browser ist opt-in); automatisierte Checkpoint-A/B- und Phase-3A–6A.2-Verträge grün. World ohne Activity, Preview und Handoff bleiben für Checkpoint C sichtprüfungsrelevant.`
...
| 6B Client World Projection | 🟨 aktiv | world-spezifische replizierte Client-Projektion; kein zweiter `clientPresentationStep()`. |
| – Checkpoint C | 🟨 aktiv | Automatisierte Verträge grün; manuelle Prüfung von World ohne Activity, Preview und Handoff folgt nach 6B. |
...
| `ArenaScene.ts` Umfang | Phase 2A: 5685 → 5570 (−115); Phase 2B: 5570 → 4715 (−855); Phase 3A: 4715 → 4530 (−185); Phase 3B: 4530 → 4204 (−326); Phase 4A: 4204 → 3782 (−422); Phase 4B: 3782 → 3670 (−112); Phase 4C: 3670 → 3466 (−204); Phase 6A.1: 3466 → 3379 (−87); Phase 6A.2: 3379 → 3061 (−318). `WorldPresentationFrameBinding.ts`: 51 → 195 (6A.1) → 541 (6A.2). |
| `WorldPresentationFrameBinding` | Trägt die World-Kamera-Positionierung (pro Frame zweimal), Surface- und Shadow-Residency/Readiness, Canopy-Transparenz, lokale Player-/Persistent-Base-Visuals sowie allgemeine World-Shadows und -Lighting inklusive Zug-/Projektillichtern. Der Binding erzeugt und räumt seine World-spezifische Zug-Occluderquelle owner-sicher auf. |
...
**Phase 6B – Clientseitige World-Projektion verschieben** (Checkpoint A/B bleiben parallel für die manuelle Sichtprüfung offen).

- Die generischen World-/Player-/Projectile-/PowerUp-/Construction-/Persistent-Base-Anteile aus `syncClientWorldSnapshotPresentation(...)` an den bestehenden World-Presentation-Owner geben.
- Coop-/Mission-/Enemy-/Objective-spezifische Projektion für Phase 7 markieren; keinen zweiten `clientPresentationStep()`-Aufruf und keinen parallelen Client-Projection-Step einführen.
...
- Danach Checkpoint C mit World ohne Activity, Preview und Handoff durchführen; die automatisierten Verträge sind bereits grün, die manuelle Sichtprüfung bleibt offen.
- `applyCameraFeedback`, `resolveWorldGradeInputs` und `resetRenderersForWorldPresentationTeardown` bleiben bis zu einer jeweils passenden späteren Ownership-Prüfung außerhalb der Phase 6A.2. Das ungenutzte `useWorldCamera` wird in Phase 9 oder einer dafür beschlossenen Vertragsänderung zusammengeführt.
