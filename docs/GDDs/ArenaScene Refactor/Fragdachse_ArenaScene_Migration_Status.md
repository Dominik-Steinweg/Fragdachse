...
**Gesamtstatus:** `✅ Phase 9 plus gezielte Nachkonsolidierung abgeschlossen – automatisierter Architektur-Gate grün; Checkpoint A/B/C/D warten weiterhin auf manuelle Sichtprüfung`
**Letzter verifizierter Repository-Stand:** `main` nach gezielter Nachkonsolidierung des Phase-9-Stands
**Automatisierter Gate für dieses Refactoring:** `npm run check` grün (342 Testdateien, 2887 Tests, 15 skipped; `tsc` + `vite build` erfolgreich)
**Manueller Gate:** `offen – visuelle Prüfung nicht ausgeführt (Browser ist opt-in); automatisierte Checkpoint-A/B/C/D- und Phase-3A–9-Verträge grün. World ohne Activity, Preview, Handoff und Activity-A→B-Rebinding bleiben sichtprüfungsrelevant.`
...
| 9 Cleanup / Final Gate | ✅ abgeschlossen | Compatibility-Fassaden, ungenutzte Scene-Annahmen und die verbliebene konkrete Coop-Presentation-Liste aus `ArenaScene` entfernt; finale Source-Ratchets und `npm run check` grün. |
| Nachkonsolidierung nach Phase 9 | ✅ abgeschlossen | `WorldCombatGameplayBinding` ist nach `destroy()` auch an Enemy-/PowerUp-Mutationspfaden inert; Homing-/Spawn-Targeting liegt an World-Combat-Composition/Binding; Aim-/Placement- und allgemeine Combat-/FX-Frame-Presentation liegen in fokussierten scene-langlebigen Ownern. |
...
Stand: 17 Dateien / 33 Assertion-Stellen; die Phase-6B-Ratchetts prüfen zusätzlich den neuen `ArenaRuntime`-/`WorldPresentationFrameBinding`-Pfad, Phase 8 den Top-Level-Frame-Cutover und Phase 9 sowie die Nachkonsolidierung die Abwesenheit veralteter Scene-Ownership. Weitere Pfadschreibweisen, Glob- oder Verzeichnis-Scans auf die Scene existieren nicht (per `rg` über `readFileSync`, `readdirSync`, `src/scenes` in `tests/` verifiziert).
...
| 19 | `WorldMetricsScopeContracts` · Respawn-Kontext aus der aktiven World | B | Spawn-Map aus `world.descriptor.definitionId`, nicht aus Rundenstate | World-Combat-Composition | `ArenaWorldCombatComposition` / `WorldCombatGameplayBinding` | Nachkonsolidierung |
...
| 23 | `ArenaSceneFrameCutover` · keine langen fachlichen Update-Blöcke | R | `ArenaScene.update()` zeigt die grobe Frame-Reihenfolge; benannte Scene-Schritte kapseln Lobby-, Rollen-, World-ohne-Activity-, Effekt- und Aim-Orchestrierung | `ArenaScene.update()` | `ArenaScene` + scene-langlebige Presentation-Owner | 8 / Nachkonsolidierung |
| 24 | `ArenaSceneFrameCutover` / `WorldGameplayCompositionContracts` · Ownership-Ratchets der Nachkonsolidierung | R | Keine Homing-Provider, konkreten Combat-/FX-Listen oder tiefen Aim-/Placement-Updates in `ArenaScene`; Targeting-/Spawn-Verdrahtung an World-Combat-Grenze | `ArenaScene` / World-Composition | `WorldCombatGameplayBinding`, `ArenaAimPresentationController`, `ArenaCombatPresentationController` | Nachkonsolidierung |
...
**Ergebnis 9.** Die verbliebenen World-/Activity-Kompatibilitätsgetter und der nicht mehr verwendete `CoopDefenseProgressSnapshot`-Import wurden aus `ArenaScene` entfernt. Das Audit fand zusätzlich die noch dort verbliebene konkrete Coop-Objective-/Map-Event-/World-space-Presentation-Liste; ihre scene-langlebige Infrastruktur und das UI-Port-Literal liegen jetzt im fokussierten `CoopMissionPresentationInfrastructure`-Owner. `CenterHUD` erhält den Announcement-Port über eine schmale Bind-Methode; Activity-State und der kanonische Activity-Step bleiben beim bestehenden `CoopMissionPresentationBinding`-/Lifecycle-Pfad. Die neue Source-Ratchet-Prüfung stellt sicher, dass die konkrete Liste nicht in `ArenaScene` zurückkehrt. Die finalen Architektur-Ratchets schützen weiterhin die Network-Consumer-Grenze, die World-/Activity-Binding-Lifetime, den einmaligen Activity-Client-Step, die Gameplay-/Physics-Freiheit des World-Presentation-Bindings und das Ausbleiben eines God-Composers. Verhalten, Authority und Lifetime-Grenzen wurden nicht verändert.

**Ergebnis der gezielten Nachkonsolidierung.** `WorldCombatGameplayBinding.updateEnemyManager()` und `.setPowerUpSystem()` prüfen jetzt den Destroy-Zustand; der Stale-Regressionstest beobachtet beide Mutationspfade. Homing-Zielauflösung einschließlich Turrets, Spieler, Decoys, Enemies, Bases, Burrow-/Damage-/Validity-/Line-of-Fire-Regeln sowie der World-abhängige Spawn-/Threat-Kontext liegen jetzt an der bestehenden World-Combat-Composition und werden vom World-Binding gebunden bzw. beim Teardown gelöst. `ArenaAimPresentationController` besitzt als scene-langlebiger, ausschließlich darstellender Owner Aim/Cursor, Scope, Charge-/Targeting-/Placement-/Tunnel- und Gauss-Präsentation; `ArenaCombatPresentationController` sequenziert die allgemeine scene-langlebige Combat-/FX-Präsentation. Beide erhalten schmale Ports und immutable Frame-Inputs; `WorldPresentationFrameBinding`, `CoopMissionPresentationBinding` und der kanonische Activity-Client-Step bleiben unverändert in ihrer Lifetime. `ArenaScene.update()` zeigt nur noch die beiden benannten Presentation-Schritte; die konkrete Fachlogik und Rendererlisten liegen außerhalb der Scene.
...
| R-3 | Stale World Binding | Ein bereits zerstörter Binding darf keine scene-langlebigen Renderer-/Lighting-/Occluder-/Listener-Consumer einer nachfolgenden World verändern. | `WorldGeometryBindingLifecycle`, `WorldCombatGameplayBinding` und `WorldPresentationFrameLifetime` schützen die jeweiligen Consumer gegen stale Mutationen; der owner-sichere `LightingSystem.clearDynamicOccluderSource()` verhindert, dass ein alter World-Teardown die Occluderquelle der nächsten World entfernt. **Geschlossen:** `ActivityRebindingContracts` prüft `WorldCombatGameplayBinding` inklusive `setPowerUpSystem()` und `updateEnemyManager()` nach `destroy()`. |
...
| Source-Test-Inventar `rg` über `tests/` | 17 Dateien / 33 Assertion-Stellen lesen `src/scenes/ArenaScene.ts` als Text; keine weiteren Pfadschreibweisen oder Verzeichnis-Scans. |
...
| `npm run check` (nach Phase-9-Auditkorrektur) | grün – 341 Testdateien, 2880 Tests, 15 skipped; `tsc` und `vite build` erfolgreich. Die drei bekannten Vite-Font-Auflösungswarnungen bleiben unverändert. |
| `npm run check` (nach gezielter Nachkonsolidierung) | grün – 342 Testdateien, 2887 Tests, 15 skipped; `tsc` und `vite build` erfolgreich. Die drei bekannten Vite-Font-Auflösungswarnungen bleiben unverändert. |
| `ArenaScene.ts` Umfang | Phase 2A: 5685 → 5570 (−115); Phase 2B: 5570 → 4715 (−855); Phase 3A: 4715 → 4530 (−185); Phase 3B: 4530 → 4204 (−326); Phase 4A: 4204 → 3782 (−422); Phase 4B: 3782 → 3670 (−112); Phase 4C: 3670 → 3466 (−204); Phase 6A.1: 3466 → 3379 (−87); Phase 6A.2: 3379 → 3061 (−318); Phase 6B: 3061 → 3031 (−30); Phase 7A.1: 3031 → 2952 (−79); Phase 7A.2: 2952 → 2950 (−2); Phase 7B: 2950 → 2953 (+3); Phase 8: 2953 → 3129 (+176); Phase 9: 3129 → 3010 (−119); Nachkonsolidierung: 3010 → 2757 (−253). `ArenaScene.update()` umfasst 263 Zeilen und enthält keine langen fachlichen Update-Blöcke mehr; `ArenaAimPresentationController.ts`: 255 Zeilen; `ArenaCombatPresentationController.ts`: 91 Zeilen; `WorldPresentationFrameBinding.ts`: 51 → 195 (6A.1) → 541 (6A.2) → 653 (6B); `CoopMissionPresentationBinding.ts`: 211 (7A.1) → 267 (7A.2) → 311 (7B); `CoopMissionPresentationInfrastructure.ts`: 236 (Auditkorrektur). |
...
**Finaler manueller Gate – Checkpoint A/B/C/D.** Die automatisierte Migration einschließlich Phase 9 und der gezielten Nachkonsolidierung ist abgeschlossen.
...
- `applyCameraFeedback`, `resolveWorldGradeInputs`, `resetRenderersForWorldPresentationTeardown` und das ungenutzte `useWorldCamera` bleiben als bewusste Ownership-/Teardown-Prüfpunkte außerhalb des abgeschlossenen Cleanup-Scope dokumentiert; ebenso verbleiben die Top-Level-Transitions, Camera-/Readiness-Reihenfolge und scene-langlebige Renderer-Komposition bewusst in `ArenaScene`.
