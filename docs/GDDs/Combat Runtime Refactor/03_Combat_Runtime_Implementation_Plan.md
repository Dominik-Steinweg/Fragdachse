...
**Reviewer:** eigener frischer Kontext und keine Produktions-, Test-, Status- oder Konfigurationsänderungen. Er prüft den realen Code gegen Verträge, nicht nur die Implementierungszusammenfassung. Findings benötigen Datei/Symbol, Auswirkung, Repro bzw. überprüfbare Argumentation, betroffenen §/V-Nachweis und Schwere. Erwartete spätere Anschlüsse sind keine Vertragsfehler. Das Urteil lautet bestanden, nicht bestanden oder nicht ausreichend verifiziert. Es erteilt niemals eine Nutzerfreigabe.

Ein Reviewer im Read-only-Modus fordert nötige Testläufe beim Orchestrator an, falls der Runner Cache-/Build-Dateien schreiben muss. Der Orchestrator kann sie selbst ausführen, ohne den Produktionsstand zu verändern. Ein blockierter Test wird nicht als bestanden interpretiert.
...
Normale lokale Korrekturen erfolgen beim zuständigen Worker. Nach höchstens zwei erfolglosen gezielten Reparaturversuchen **desselben** Problems wird mit kleinem Repro/Contract-/Diff-Paket nach 05 eskaliert. Ein gezielter höherstufiger Diagnose-/Reparaturauftrag ist erlaubt; bleibt Ursache oder Vertrag danach ungeklärt, hält der Block an. Keine unbeschränkte Schleife durch immer neue Agenten, Reset der Versuchszählung oder Hochstufen aller Folgephasen.

Neue grundlegende Ownership-/Lifetime-/Provenance-Fragen, gefährdete Nachbarcontracts oder notwendige Änderungen an 01/02 gehen unabhängig von der Versuchszahl zur Nutzerentscheidung. Lokale Fixes innerhalb klarer bestehender Verträge brauchen dagegen keine manuelle Einzelabnahme. R1/R2 dürfen solche Fixes durch einen Worker durchführen lassen und einmal gesammelt erneut prüfen. Nach zwei nicht bestandenen Review-Runden wird der Block mit dem verbleibenden Befund beendet.

Für P13 gilt dasselbe begrenzte Review-/Fix-Prinzip. Der Reviewer beurteilt, ein passender Implementierungsagent korrigiert, der unabhängige Reviewer prüft den geänderten Endstand. Der Orchestrator darf ein negatives Review nicht selbst in einen Pass umdeuten.
...
P13 ist die vom Orchestrator geführte Abschlussphase mit **separatem Read-only-Reviewer**. Reproduzierbare Findings werden an den nach 05 passenden Worker gegeben; der Reviewer repariert nicht seine eigene Prüfgrundlage. Nach Fixes betroffene Nachweise und unabhängiges Review erneut prüfen. Anschließend muss die vollständige Gate-Matrix auf dem **finalen Produktionscode** bestehen; bereits auf exakt diesem Code erfolgreiche Gates nicht nochmals laufen lassen.
