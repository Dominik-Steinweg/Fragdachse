...
export interface WorldPowerUpRuntimeOptions {
  ...
  readonly onPickupCollected?: PowerUpSystemOptions['onPickupCollected'];
  ...
  private createSystemOptions(): PowerUpSystemOptions {
    return {
      onPickupCollected: (playerId, pickup) => {
        this.options.onPickupCollected?.(playerId, pickup);
        this.options.recordPowerUpCollected(playerId);
      },
