...
export type PlayerCombatResourcePort = Pick<
  ...
  | 'drainAdrenaline'
  | 'addAdrenaline'
  | 'captureAdrenalineGainBasis'
  | 'resolveAdrenalineGain'
  | 'commitResolvedAdrenalineGain'
  | 'getMaxAdrenaline'
  | 'getAdrenalineRevision'
  | 'refundAdrenaline'
  | 'addRage'
...
export interface PlayerCombatReactionPort {
  ...
  handleDirectAk47EnemyHit(
    context: ProjectileAk47HitContext,
    enemyId: string,
    nowMs: number,
  ...
  handleCoopDefenseItemKill(killerId: string, victimId: string, x: number, y: number, nowMs: number,
    origin?: { readonly kind: CombatDamageKind; readonly slot?: LoadoutSlot }): void;
