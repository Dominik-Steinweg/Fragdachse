...
import type { PlayerAimNetState, PlayerNetState, GamePhase, PlayerProfile, WeaponSlot, RoomQualitySnapshot, RoomQualityStatus } from '../types';
import type { HostRoomQualityProbeResult, RoundResult } from '../network/NetworkBridge';
import {
  ARENA_HEIGHT,
...
export class ArenaScene extends Phaser.Scene {
  ...
  private hostSoloProbeBrowserRttMs: number | null = null;
  ...
  private hostCheckReadyToStart(): void {
    ...
    if (this.roomQualitySnapshot?.startBlocked && this.roomQualitySnapshot.status !== 'good') return;
  ...
  private initializeRoomQuality(): void {
    ...
    this.nextRoomQualitySampleAt = this.time.now + ROOM_QUALITY_SAMPLE_INTERVAL_MS;
    ...
    this.hostSoloProbeBrowserRttMs = null;
    this.hostSoloProbeSampleCount = 0;
    this.hostSoloProbeAttempted = false;

    if (!bridge.isHost()) {
      this.roomQualitySnapshot = bridge.getRoomQuality();
      return;
    }

    this.publishRoomQuality(this.buildRoomQualitySnapshot('sampling', 'Host-Probe startet…', null, 0, 0, 0, 'host-proxy'));
  ...
  private updateRoomQuality(now: number, players: PlayerProfile[]): void {
    ...
    const remotePlayers = players.filter(player => player.id !== bridge.getLocalPlayerId());
    ...
    if (now >= this.nextRoomQualitySampleAt) {
      ...
      for (const player of remotePlayers) {
        const ping = bridge.getPlayerPing(player.id);
        if (ping <= 0) continue;
    ...
    const retryCount = getRoomQualityRetryCount();
  ...
  private updateHostSoloRoomQuality(now: number, totalPlayers: number): void {
    ...
    if (this.hostSoloProbeInFlight || !this.hostSoloProbeAttempted) {
      this.autoRetryAtMs = null;
      this.autoRetryTriggered = false;
      this.publishRoomQuality(this.buildRoomQualitySnapshot('sampling', 'Host-Probe misst Raumqualitaet…', null, 0, totalPlayers, 0, 'host-proxy'));
      return;
    }
    ...
    if (this.hostSoloProbeEstimateMs === null) {
      this.autoRetryAtMs = null;
      this.autoRetryTriggered = false;
      this.publishRoomQuality(this.buildRoomQualitySnapshot('waiting', 'Host-Probe ohne Ergebnis - Link kann trotzdem geteilt werden', null, 0, totalPlayers, 0, 'host-proxy'));
      return;
    }
    ...
    if (this.hostSoloProbeEstimateMs <= ROOM_QUALITY_MAX_ACCEPTABLE_PING_MS) {
      this.autoRetryAtMs = null;
      this.autoRetryTriggered = false;
      this.publishRoomQuality(this.buildRoomQualitySnapshot('good', `Host-Probe ok: ${summaryDetails}`, this.hostSoloProbeEstimateMs, 0, totalPlayers, this.hostSoloProbeSampleCount, 'host-proxy'));
      return;
    }
    ...
    if (ROOM_QUALITY_RETRY_MODE === 'auto' && retryCount < ROOM_QUALITY_MAX_AUTO_RETRIES) {
      ...
      this.publishRoomQuality(this.buildRoomQualitySnapshot('retrying', `Host-Probe schlecht: ${summaryDetails} - neuer Raum folgt`, this.hostSoloProbeEstimateMs, 0, totalPlayers, this.hostSoloProbeSampleCount, 'host-proxy'));
      if (!this.autoRetryTriggered && now >= this.autoRetryAtMs) {
        this.autoRetryTriggered = true;
        restartRoomForQualityRetry();
      }
    ...
    const retryHint = retryCount >= ROOM_QUALITY_MAX_AUTO_RETRIES
      ? ' - Auto-Retry-Limit erreicht'
      : ' - neuer Raum empfohlen';
    this.publishRoomQuality(this.buildRoomQualitySnapshot('bad', `Host-Probe schlecht: ${summaryDetails}${retryHint}`, this.hostSoloProbeEstimateMs, 0, totalPlayers, this.hostSoloProbeSampleCount, 'host-proxy'));
  }

  private async startHostSoloProbe(): Promise<void> {
    if (!bridge.isHost() || this.hostSoloProbeInFlight) return;
    this.hostSoloProbeInFlight = true;
    this.hostSoloProbeAttempted = true;
    try {
      const result: HostRoomQualityProbeResult = await bridge.measureHostRoomLoopback(
        ROOM_QUALITY_HOST_PROBE_SAMPLE_COUNT,
        ROOM_QUALITY_HOST_PROBE_TIMEOUT_MS,
      );
      this.hostSoloProbeEstimateMs = result.estimateMs;
      this.hostSoloProbeLoopbackMs = result.loopbackAverageMs;
      this.hostSoloProbeBrowserRttMs = result.browserRttMs;
  ...
  private buildRoomQualitySnapshot(
    ...
    return {
      ...
      retryCount: getRoomQualityRetryCount(),
  ...
  private publishRoomQuality(snapshot: RoomQualitySnapshot): void {
    ...
    bridge.publishRoomQuality(snapshot);
  }
}
