...
function isMovementPathBlocked(
  ...
  const minGridX = Math.floor((Math.min(worldX, endX) - radius - ARENA_OFFSET_X) / CELL_SIZE);
  const maxGridX = Math.floor((Math.max(worldX, endX) + radius - ARENA_OFFSET_X) / CELL_SIZE);
  const minGridY = Math.floor((Math.min(worldY, endY) - radius - ARENA_OFFSET_Y) / CELL_SIZE);
  const maxGridY = Math.floor((Math.max(worldY, endY) + radius - ARENA_OFFSET_Y) / CELL_SIZE);
