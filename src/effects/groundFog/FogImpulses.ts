...
export interface FogImpulse {
  ...
  strength: number; kind: 'motion' | 'projectile' | 'explosion'; priority: number;
}
...
export class FogImpulses {
  ...
  add(input: FogImpulse): void {
    ...
    const key = `${input.kind}:${Math.floor(input.x / size)},${Math.floor(input.y / size)}:${Math.floor(input.endX / size)},${Math.floor(input.endY / size)}`;
