...
export function referenceMap(mapId = PERFORMANCE_MAP_ID): CoopDefenseMapConfig {
  ...
  return { mapId, arenaWidthCells: 160, arenaHeightCells: 96, timeOfDay: voidFire ? '20:30' : '12:00',
    ...
    ].filter(wall => !voidFire || wall.id.endsWith('-reserve')), surviveDurationSec: 3600, balanceReferenceDurationSec: 3600, objective: 'survive', respawnsPerPlayer: 0,
    ...
    mapEvents: voidFire ? [{ id: 'reference-void-fire', type: 'ground-hazard', start: { type: 'time', atMs: 0 }, delayMs: 0,
      area: fixture.voidFire.area, spread: fixture.voidFire.spread, effect: fixture.voidFire.effect }]
      : mapId.endsWith('-train') ? [{ id: 'reference-train', type: 'train', start: { type: 'time', atMs: 5000 }, delayMs: 0 }] : [],
  };
}
