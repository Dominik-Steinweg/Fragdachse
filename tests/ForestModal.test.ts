...
describe('forest modal decoration', () => {
  ...
  it('places noninteractive decoration above content and below transient layers, unregistering on destruction', () => {
    const { scene } = fixture();
    for (let i = 0; i < 4; i++) {
      ...
      const frame = mountForestModal(scene, root, 780, 700, [ghost as never, tooltip as never]);
