...
import type { ProjectileLifecycleOutcome } from '../src/projectile/ProjectileGameplayPort';
...
function projectile(shotId: number, overrides: Partial<ProjectileAk47HitContext> = {}): ProjectileAk47HitContext {
  return {
    ownerId: 'p1',
    shotId,
...
function resolvedProjectile(shotId: number, fireSuperiorityShot = false): ProjectileLifecycleOutcome {
  return {
    kind: 'resolved',
    projectileId: shotId,
    provenance: createSingleOwnerProvenance('p1', { weaponSourceId: 'weapon.ak47', sourceSlot: 'weapon2' }),
    reaction: {
      ak47: {
        shotId,
        fireSuperiorityShot,
...
describe('AK-47 Coop-Defense-Upgradebaum', () => {
  ...
  it('builds one shared hit chain, protects full stacks during breakthrough, and does not retrigger while pending', () => {
    ...
    const miss = resolvedProjectile(6);
    ...
    behavior.resolveProjectile(resolvedProjectile(99, true));
  ...
  it('refunds the concrete strategic penetrator once and ends only after pending resolves', () => {
    ...
    const shot = resolvedProjectile(10, true);
