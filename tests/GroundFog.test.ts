...
import { createWeaponFogTrailProfile, weaponFogTrail } from '../src/effects/groundFog/WeaponFogTrail';
import { WEAPON_CONFIGS, UTILITY_CONFIGS } from '../src/loadout/LoadoutConfig';
...
describe('ground fog bounded world state', () => {
  it('resolves factors by exact weapon identity and scales width and the entire fade independently', () => {
    ...
    expect(weaponFogTrail('unknown')).toEqual({});
    const base = createWeaponFogTrailProfile(10, .5, {})!;
    const wider = createWeaponFogTrailProfile(10, .5, { fogTrailWidthFactor: 2 })!;
    const longer = createWeaponFogTrailProfile(10, .5, { fogTrailDurationFactor: 2 })!;
    expect(wider).toEqual({ ...base, radius: base.radius * 2 });
    expect(longer).toEqual({ ...base, lifeMs: base.lifeMs * 2, decayMs: base.decayMs * 2 });
    expect(createWeaponFogTrailProfile(10, .5, { fogTrailWidthFactor: 0 })).toBeNull();
    expect(createWeaponFogTrailProfile(10, .5, { fogTrailDurationFactor: 0 })).toBeNull();
    const fog = new GroundFogSystem({ sys: { renderer: { on: vi.fn(), off: vi.fn() } } } as never, frame, 1, []);
    const point = { x: 300, y: 300, timeMs: 0, sequence: 1, vx: 0, vy: 0 };
    const disabled = { fogTrailDurationFactor: 0 };
    fog.addProjectile({ from: point, to: { ...point, x: 500 }, ageMs: 0 }, 60, 'bfg', 1, disabled);
    fog.addHitscan(300, 300, 500, 300, 12, disabled);
    fog.addMelee(300, 300, 0, 90, 100, disabled);
    expect(fog.getDiagnostics().pendingImpulses).toBe(0); fog.destroy();
  });
  it('keeps melee sectors distinct and applies the projectile quality gate to hitscan', () => {
    ...
    expect(fog.getDiagnostics().pendingImpulses).toBe(2);
    fog.freeze();
    fog.quality = 'low'; fog.addHitscan(300, 300, 900, 300, 2); expect(fog.getDiagnostics().pendingImpulses).toBe(0);
    fog.addMelee(300, 300, 0, 90, 100); expect(fog.getDiagnostics().pendingImpulses).toBe(1); fog.freeze();
    fog.enabled = false; fog.addMelee(300, 300, 0, 90, 100); expect(fog.getDiagnostics().pendingImpulses).toBe(0);
    fog.enabled = true; fog.reactions = false; fog.addHitscan(300, 300, 900, 300, 2); expect(fog.getDiagnostics().pendingImpulses).toBe(0);
    fog.destroy();
  });
