...
describe('Inspector tool radial geometry', () => {
  ...
  it('resolves clockwise segments from twelve o clock without negative indices', () => {
    expect(getRadialMenuSegmentIndex(0, -80, 4, 34)).toBe(0);
    expect(getRadialMenuSegmentIndex(80, 0, 4, 34)).toBe(1);
    expect(getRadialMenuSegmentIndex(0, 80, 4, 34)).toBe(2);
  ...
  it('supports the complete six-slot Inspector capacity', () => {
    ...
    expect(directions.map(([x, y]) => (
      getRadialMenuSegmentIndex(x, y, 6, 34)
    ))).toEqual([0, 1, 2, 3, 4, 5]);
  });
});
