...
function fixture() {
  ...
  const owner = new WorldCombatReactions({
    ...
    combatSystem: { applyDamage: damage } as never,
...
describe('World Combat reaction policies', () => {
  it('does not re-reflect reflected damage even when an item proposes another reflection', () => {
    const f = fixture();
    f.owner.handlePlayerDamageTaken('credited', 'source', 10, 0, 'reflect', 1000, () => true);
    expect(f.damage).not.toHaveBeenCalled();
    f.owner.handlePlayerDamageTaken('credited', 'source', 10, 0, 'direct', 1000, () => true);
